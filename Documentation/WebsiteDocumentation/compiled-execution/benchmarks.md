...
**Follow-up optimization after v4.4.0.** A subsequent qualification of constant-stratification MATLAB compiled execution measured composite integration at **49.120 → 41.770 s (15.0% less time)** against an earlier batching-only optimization baseline. The coefficient endpoint median was 14.609 → 14.379 s. Each selected median uses three fresh processes on the same Apple M4 Max; all timing observations and numerical checks are retained in the [qualification report](https://github.com/JeffreyEarly/wave-vortex-model/blob/078f58f5ab0b2d84ce18b9f89a61042579006d4e/Benchmarks/CONSTANT-VERTICAL-CALCULUS-FOLLOWUP.md). These runs used reconstructed fixtures and did not collect a new three-interface comparison or peak-memory campaign. The release table below retains its original v4.4.0 measurements; replacing it requires a new matched publication cohort.

**Tracer derivative follow-up for v4.4.1.** A focused Hydrostatic exponential qualification on the Apple M4 Max measured MATLAB compiled composite integration at **53.757 → 32.024 s (40.4% less time)**. The coefficient endpoint median was 12.462 → 12.457 s. Each median uses three fresh MATLAB processes with frozen source, MEX and FFTW provider identities; every output graph passed the existing numerical tolerances and integration-work checks. The implementation uses axis-only horizontal FFTs and prepared real vertical matrices, including the balanced F/G tracer calculus in Boussinesq. The measured speedup here is for Hydrostatic; a new Boussinesq speedup is not inferred from it. See the [tracer qualification report](https://github.com/JeffreyEarly/wave-vortex-model/blob/main/Benchmarks/TRACER-DERIVATIVES.md) for all samples, variability, source identities and diagnostic tracer attribution. These runs used reconstructed fixtures matching the archived initial scientific values, and do not replace the original release cohort or its MATLAB builtin, standalone and peak-memory results.
