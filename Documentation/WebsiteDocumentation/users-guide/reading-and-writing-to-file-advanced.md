...
Output initialization is transactional. If a group cannot initialize, WaveVortexModel closes and removes the newly created partial file and resets the output objects so the operation can be retried. If a later time-step write fails, all model-owned file handles are closed and the existing file is preserved for inspection. Each output group stages its payload, synchronizes it, and then commits the record by writing a finite value to the unlimited `t` coordinate. Restart accepts only the contiguous finite prefix of `t`, rejects a finite time after a fill-valued hole, and overwrites the first uncommitted payload slot when output resumes.
