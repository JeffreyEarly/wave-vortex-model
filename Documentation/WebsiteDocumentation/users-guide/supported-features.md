...
The unreleased integration branch contains an experimental constant-stratification `fastTransform="fftw"` implementation. It uses the source-only `FFTWTransforms ^1.0.2` package and locally built MEX modules only when its MATLAB-bundled r2c/c2r capability, ownership, and numerical checks pass. The backend is not a supported release feature unless the fixed issue #47 end-to-end timing, correctness, and memory gates all pass. Published WaveVortexModel releases therefore retain builtin transforms as the supported default. Variable-stratification and quasigeostrophic transforms remain builtin-only.
