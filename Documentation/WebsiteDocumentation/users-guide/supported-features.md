...
| FFTW and `RealToComplexTransform` integration | Internal | Runtime selection and supporting classes are retained for future development but are not selectable through a supported transform contract. |
...
| `exact` and `finufft` interpolation | Internal | The low-level barotropic FINUFFT path is retained for development, but these option values must not appear in public interpolation lists or documentation. |
| `WVOffGridTransform`, external-wave behavior, and off-grid pressure | Internal | These incomplete remnants are preserved only in the authoring legacy archive and have no 4.2.x compatibility promise. |
...
The Internal forcing classes are preserved only in the authoring legacy archive; they are not available on the runtime package path.
...
| Corrected low-level adaptive-integrator and doubly-periodic validator names | Internal | These implementation names have no compatibility promise; the stable `WVModel` facade remains unchanged. |
| `shouldExcludeConjugates` geometry setting | Internal | New source and files use the corrected spelling. Version 4.x silently reads the legacy `shouldExludeConjugates` persisted field when the corrected field is absent; this fallback is removed in version 5 under issue #54. |
