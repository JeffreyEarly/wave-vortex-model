...
The committed [`barotropic-qg-v1.json`](https://github.com/JeffreyEarly/wave-vortex-model/blob/feature/v4.3-portable-runtime/PortableRuntime/qualification/barotropic-qg-v1.json) record maps the supported QG surface to its executable MATLAB and C++ qualification gates. Those gates use requests written by `WVModel.writePortableRunRequest`, compare compact coefficients, fields, invariants, observer state, schedules, and NetCDF schemas through a MATLAB--C++--MATLAB round trip, and require relative coefficient and field errors no larger than $$10^{-12}$$. The record contains coverage and acceptance limits rather than benchmark samples.
...
Narrow-band QG forcing executes through the fixed-amplitude numerical contract while retaining its MATLAB provenance variables (`r`, `k_r`, `k_f`, `j_f`, `u_rms`, and `initialPV`) in every created or replaced NetCDF file. Returning to MATLAB therefore restores the original `WVNarrowBandGeostrophicForcing` object rather than a generic fixed-amplitude substitute.
