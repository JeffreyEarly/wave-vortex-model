...
`WVObserverOutputEvaluationService` implements the passive `WVCoefficients`, `WVEulerianFields`, and `WVMooring` evaluation boundary. It compiles one field plan from all configured observers, coalesces identical field and sampling requests, evaluates that plan once for each coincident output event, and returns borrowed coefficient views or bounded caller-independent field buffers to the persistence sink. Moorings retain MATLAB's periodic coordinate wrapping and one-based nearest-horizontal-grid convention. Their static coordinate variables are written transactionally with the file schema and are not repeated at each output time.
