...
Canonical `Ap`, `Am`, and `A0` remain three caller-owned complex `[Nj,Nkl]` arrays. The C++ numerical core sees the same zero-copy `WVState` views used before composite state was introduced. Additional integrated state is an ordered sequence of named real or complex blocks. Each block records its natural dimensions, ownership, absolute-tolerance policy, and restart requirement. Every noncanonical integrator-owned block has exactly one numerical owner: one `WVLagrangianParticles` or `WVTracer` observer. Orphan blocks and blocks shared by multiple integrated observers are rejected when the descriptor is frozen. The same observer may still be routed to any number of output groups and files because routing does not create another numerical owner.
