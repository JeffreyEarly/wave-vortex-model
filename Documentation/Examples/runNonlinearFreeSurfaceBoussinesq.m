...
% model. The direct manuscript RHS uses modal pressure to quadratic order.
% Inspect physical energy and its actual rate; finite retained inventories
% need not conserve this diagnostic exactly.
...
for time=0:20:200
    ...
    row=struct(time=time,totalEnergy=stage.totalEnergy,energyTendency=stage.energyTendency,prescribedWork=stage.prescribedWork,minimumLabel=stage.minimumLabel,maximumLabel=stage.maximumLabel);
...
nexttile; plot(history.time,history.energyTendency,'LineWidth',1.5); grid on;
xlabel('Time (s)'); ylabel('Energy rate (m^3 s^{-3})'); title('Actual resolved energy rate');
nexttile; plot(history.time,[history.minimumLabel+wvt.Lz,-history.maximumLabel],'LineWidth',1.5); grid on;
xlabel('Time (s)'); ylabel('Minimum parcel-label clearance (m)'); title('Reference-domain margins'); legend('Bottom','Surface',Location='best');
