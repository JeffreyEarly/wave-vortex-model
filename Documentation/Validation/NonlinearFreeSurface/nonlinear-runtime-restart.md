...
The control covers 40 seconds and one qualified truncation. It does not establish a timestep convergence rate, long-time stability, general stratification, conservation of material/APV moments, or convergence of pressure along the reduced trajectory. It preserves and compares the existing instantaneous pressure diagnostic. This first control omits particles and tracers; the nonlinear observer continuation control below complements the separate instantaneous coordinate tests. Optional user-directory, undamped-model, and intermediate package-path-removal warnings did not prevent the test from completing.
...
## Nonlinear particle and tracer continuation

`TestFreeSurfaceNonlinearRestart/nonlinearParticleAndTracerContinuationAgree` advances the same qualified mixed Boussinesq fixture with explicit nonlinear advection and a prescribed physical source from t=327 to 347 s, with t0=-17 s and fixed RK4 steps of 5 s. It integrates two particles with physical three-dimensional positions, a nonconstant reference-grid tracer, and a constant tracer alongside the evolving coefficient state. A second run writes a native checkpoint at 337 s, restores a new WVModel with empty `verticalModes`, and continues to 347 s.

The test checks exact observer values at the checkpoint and compares final particle positions, both tracers, six-family model state and clocks against uninterrupted evolution. It also checks that particles remain within the physical column, move horizontally and vertically, and that the nonconstant tracer changes. The constant tracer is checked at the checkpoint and both final states. This establishes observer continuation under the nonlinear runtime, complementing the independent instantaneous coordinate/flux tests; it does not establish long-time material conservation or trajectory convergence.

The new method alone passed on MATLAB R2025b Update 4 with the pinned beta dependency exports in 15.2115 s. Maximum particle-coordinate restart error was 0 m; maximum nonconstant-tracer restart error and constant-tracer preservation error were both 0. Maximum particle-coordinate displacement was 7.26508 m, maximum vertical displacement was 0.199265 m, and maximum nonconstant-tracer change was 0.0008646. Assertions allow 2e-9 m particle error, 2e-12 absolute tracer error, and 2e-11 relative coefficient error. Code Analyzer reported no findings in the updated test; whitespace validation passed. No runtime change or additional successful scientific test rerun was needed. Optional user-directory and undamped-model warnings did not affect completion.
