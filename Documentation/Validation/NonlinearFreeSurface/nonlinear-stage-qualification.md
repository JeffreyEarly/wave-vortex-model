...
## Bounded endpoint-roundoff correction and pure-wave regression

Root commit `f4bff206` addresses the preceding exploratory rejection by allowing excursions no greater than $32\,\mathrm{eps}(D)$ to be evaluated at the canonical endpoint. At $D=1000$ m this is approximately $3.638\times10^{-12}$ m. Raw labels remain available as `label`; `densityLabel` records the actual in-domain thermodynamic evaluation coordinate. Maximum adjustment, adjusted sample count, and tolerance are reported. Coefficients and reconstructed fields are not changed. Excursions larger than the allowance still throw. This is an explicit finite-precision convention, not an extension of parcel density or permission for finite-amplitude out-of-domain parcels.

The new `pureWaveEndpointRoundoffRemainsBoundedAndNonlinear` regression repeats the constant/exponential profiles and three clocks above, at SSH amplitudes 1 m and 0.5 m. All twelve composed stage evaluations now succeed. The test verifies that raw labels equal reconstructed $z-\eta$, effective density labels stay inside the reference interval, reported adjustments/counts match their actual arrays and stay within the declared bound, all excess-rate families are finite, and coefficients/clocks remain untouched.

The optional nonlinear-limit check requires quadratic-or-faster decay under amplitude halving; it does not require a nonzero quadratic leading term for a single wave on this restricted horizontal band. Observed excess-rate norm ratios were approximately 7.99935 for constant stratification and 8.00375–8.00453 for exponential stratification. An initial assertion demanding a ratio near four was therefore corrected: these observations are consistent with a suppressed quadratic term in this fixture, but do not establish a general cubic law. Only the new pure-wave method was run after this addition; the stage helper and updated test passed Code Analyzer. Existing finite-margin tests were not repeated.

A read-only mathematical review finds the adjusted thermodynamic values internally consistent: `effectiveEta = eta - (densityLabel - rawLabel)` equals $z-\mathrm{densityLabel}$, so buoyancy, APE, and density refer to the same adjusted parcel coordinate. There is a derivative qualification. In the activated endpoint strip, the reported `apeEta` is the thermodynamic derivative at the adjusted state, not necessarily the derivative of the composite endpoint-clamped evaluation with respect to the original raw displacement. That composite map is piecewise differentiable and can hold the effective displacement fixed while raw displacement varies inside the strip. Exact energy-gradient identities in raw coefficient coordinates are therefore not established at adjusted samples. The numerical value/state perturbation is explicitly bounded and reported; it should remain part of any future energy-budget accounting rather than being described as exact smooth dynamics at those samples. No mathematical or implementation change beyond the root's bounded convention was introduced in this test follow-up.
