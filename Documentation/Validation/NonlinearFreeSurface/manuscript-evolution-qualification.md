...
Runtime acceptance still needs supported forcing geometry and pressure bookkeeping, native/provider-free restart, energy/API semantics and the runnable model example to be checked against the replacement RHS. The old weak-runtime successes do not establish those checks for the replacement. Remove unneeded weak-stage machinery after that integration review. General nonlinear count defaults and longer-time qualification remain separate decisions; linear construction defaults are unchanged.
