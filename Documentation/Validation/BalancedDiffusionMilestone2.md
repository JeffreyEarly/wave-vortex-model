...
Inspection of that superseded Linux full log also identified an overly strict cross-kernel drag assertion: full-volume and selective endpoint contractions differed by approximately `4e-31` absolute / `4e-16` relative in zero-APV tendencies. The test now checks identical supplied-snapshot reuse and combined tendency evaluation exactly, and separately compares selective versus volume results within `16*eps` of each coefficient family's Frobenius norm. The zero mean remains exact. Both affected methods pass on R2025b with zero Analyzer findings and restored profiler configuration. This changes a floating-point implementation comparison, not the independent physical stress/trajectory tolerances or runtime equations.


The completed Linux full run at `f0ff626a516db2d7f0d2981777e3cd4d3ee17ee5` exercised all 1152 selected tests: 1151 passed, one failed, none was incomplete (3029.085 seconds test execution). All thermal/shared integration corrections passed. The sole failure was the pre-existing exact comparison between the APV constructor's Gram-error report and the separate lightweight assessment: `5.026863364295289e-5` versus `5.026863364295861e-5`, a `5.73e-18` absolute difference. Both use the defect of a unit-normalized Gram matrix, computed through distinct contraction paths. Their report-consistency check now allows `16*eps` absolute roundoff and separately requires both reports to satisfy the unchanged `gramTolerance`. Exact endpoint defaults, grid and mode counts, signed negative-mode retention and quadratic-aliasing gates remain intact. The corrected method passes on R2025b (3.59 seconds), with zero Analyzer findings and clean whitespace. Runtime code and scientific construction thresholds are unchanged. The complete hosted suite is rerun before merge.
