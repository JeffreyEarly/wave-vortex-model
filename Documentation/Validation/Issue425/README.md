...
The restored run takes **8.46 seconds** for the full linear example and **13.59 seconds** for the separate product preparation. It reserves **60,528 products**, estimates **315 MiB** of working memory within the existing 512 MiB budget, retains **12.2 MiB** of evidence and takes **17.9 ms** to assess the second map. The sampled quadrature-reference discrepancy is approximately **0.00402**, and the independent-EVP product discrepancy is **10.60**, both above the unchanged 10⁻⁴ allowance. These are reference failures, not measured errors in the retained physical model.
...
### Kappa-dependent figure correction

The restored default and its artifacts were generated from clean source `afb500ca985fd65f528d675ed71a00432a7bec3c` using the same beta.4 provider and OceanKit revision recorded above. Its [verification ledger](kappa-dependent/verification.json) checks exact reproduction of all 41 original CSV rows, all 24 prepared candidates, three covered output pages, absent error estimates on untested pages, reference-inconclusive outcomes for both maps, product/memory budgets, repeat timing and the existing-directory guard. A replay of the explicit longer-wave configuration reproduces its old count table and rejected/assessed map outcomes with stable references. The original linear output CSV is byte-for-byte identical to the provider's committed table. Comparing an in-memory MATLAB table directly with its CSV import initially failed because of serialization/type differences; comparing both exported tables succeeds without changing the scientific output.

The example passes Code Analyzer. The focused verification reruns only the changed example and explicit configuration path; the earlier 28 core advisory tests remain applicable because core assessment/preparation code is unchanged. The final PNG was visually inspected. The documentation check and repository-scope review cover this correction; no package manifest, runtime, provider or historical control artifacts changed.
