...
The bounded resolution-strategy proof of concept succeeds. A complete 257-direction thermal representation is a credible candidate where 217 APV modes plus two boundary modes were insufficient. This is a response-resolution result, not a global mode-count prescription or a whole-model performance benchmark.

Keep #353 open for integration into the shared v5 transform/evolution interfaces and short nonlinear/refinement/restart qualification. Reuse existing physical-field reconstruction, forcing, model and persistence idioms; preserve APV analysis and wave transforms. First implement one explicit complete thermal representation and forward linear evolution, then validate nonlinear departures using total reconstructed fields. Do not introduce compression, a duplicate experiment case manager or a new class hierarchy before the full representation works through those interfaces. #354 retains corrected-provider release/export, supported-example adoption and full scientific CI; #367 retains the long seasonal experiment.
