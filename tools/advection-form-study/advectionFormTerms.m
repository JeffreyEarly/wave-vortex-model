function terms = advectionFormTerms(hatted,pressure,xi,D,f,rho0,N2,buoyancyRemainder,derivative,form)
% Evaluate Appendix C terms before applying any modal source projector.
%
% Authoring comparison based on eq:projection-ready-nonlinear-advection-terms,
...
arguments (Input)
    ...
    form (1,1) string {mustBeMember(form,["divergence","advective","split","compatible"])}
end
...
N.eta = material(hatted.eta)./gamma-alpha.*(hatted.u.*sshX+hatted.v.*sshY)./gamma;
...
terms = struct(N=N,P=P,H=H,source=source,linear=linear,total=total,physical=physical,transportW=transportW);
    ...
    function value = divergence(field)
        value = material(field)-(surfaceW/D).*field;
    end

    function value = material(field)
        value = advectionTransport(field,hatted.u,hatted.v,transportW,-surfaceW/D,derivative,form);
    end
end
