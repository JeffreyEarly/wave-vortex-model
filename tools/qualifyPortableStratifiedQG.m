function report = qualifyPortableStratifiedQG(outputPath,options)
% Qualify SQG continuation and write measured, catalog-linked JSON evidence.
...
environmentNames = ["WV_STABLE_FORCING_RUNNER","WV_STABLE_FORCING_DUMP","WV_STABLE_FORCING_NATIVE","WV_SQG_EVIDENCE_FOLDER","WV_MODAL_DUMP_EXECUTABLE"];
...
if runner == ""
    ...
    runCommand("cmake --build "+shellQuote(build)+" --parallel 4 --target wave-vortex-run WVStableForcingDump WVStratifiedQGFieldDump WVStratifiedQGLifecycleProbe WVStratifiedModalDump");
    runner = fullfile(build,"wave-vortex-run");
end
for name = ["wave-vortex-run","WVStableForcingDump","WVStratifiedQGFieldDump","WVStratifiedQGLifecycleProbe","WVStratifiedModalDump"]
...
setenv("WV_SQG_EVIDENCE_FOLDER",folder);
...
for row = reshape(catalog.rows(startsWith(string({catalog.rows.configuration}),"stratified-qg")),1,[])
    if string(row.acceptance)=="supported"
        names = ["TestPortableStratifiedQG/applicableForcingTendenciesMatchMatlab","TestPortableStratifiedQG/orderedForcingTendenciesMatchMatlab"];
        rowProviders = providers;
    else
        names = "TestPortableStratifiedQG/incompatibleModelGraphsAreTransactional";
...
cases = readEvidence(folder,"sqg-continuation-*.json");
