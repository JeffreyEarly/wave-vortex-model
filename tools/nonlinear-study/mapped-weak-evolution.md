...
The helper records single observed construction, basis setup, mapped-RHS evaluation, mass/RHS assembly, and two-solve timings. In this run, construction took 2.67–3.11 s and basis setup 0.60–0.89 s per configuration. Mapped-RHS evaluation took 2.68–5.20 ms, combined mass/RHS assembly 6.82–24.82 ms, and the two dense weak solves 0.24–0.40 ms per evaluated state. These observations include warm-up and diagnostic overhead and are not a separate performance benchmark. No trajectory, nonlinear pressure recovery, broad arbitrary-stratification qualification, or exact finite-dimensional energy conservation is claimed.
...
The source/KKT feasibility study and the subsequent instantaneous weak-budget experiment were executed with their CSV outputs. The weak-budget helper uses the independently authored mapping and full-tendency helpers from commits `43ed50ef` and `74569724`. Its final run and MATLAB Code Analyzer passed; only diagnostic helpers, this note, and validation CSV files are authored in this workstream. No production evolution API or generated website source was changed. The instantaneous checks above are the current qualification boundary.
