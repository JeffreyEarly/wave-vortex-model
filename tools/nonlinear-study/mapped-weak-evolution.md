...
For each of the coarse and refined vertical/family configurations, trajectories use steps 10, 5, and 2.5 s on the fixed 4-by-4 retained horizontal grid with 8-by-8 horizontal quadrature. At every nonlinear RK stage the driver checks labels on both the quadrature grid and the common dense checkpoint grid, evaluates retained kinematics and divergence, and accumulates grid, reaction, solver, and SSH-geometry work using the same RK4 weights. Summary results are in `mapped-weak-trajectory-study.csv`; per-step direct energy and accumulated budgets are in `mapped-weak-trajectory-history.csv` under `Documentation/Validation/NonlinearFreeSurface`.
