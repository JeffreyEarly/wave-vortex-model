...
This historical note derives the initial finite-dimensional experiment with the small-pressure approximation. The implemented full-reference extension is specified in [the current runtime contract](../../Documentation/Validation/NonlinearFreeSurface/runtime-qualification.md); the old approximation evidence below is preserved. Its inputs are WVM physics at `81987767956fae5aadcb53a9df16482221f5f3e4`, manuscript `311ebf56` as identified in the planning task, and the constrained-source study in `a043a61af9a3595da1b17e823b2b6bdf5445b17f`. The candidate retains the existing APV, zero-APV, positive/negative wave, inertial, and mean-density-anomaly coefficient families. It does not introduce thermal modes or replace `WVModel`.
