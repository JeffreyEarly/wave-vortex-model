...
The base registry calls the protected `validateForcingInventory(self,forcing)` hook after resolving names and sorting priorities, before committing state or firing removal/change callbacks. The base hook is a no-op. The free-surface Boussinesq override accepts reference-coordinate prescribed sources in linear mode; physical-coordinate sources require nonlinear advection in the effective inventory. Nonlinear advection requires `shouldAntialias=true`; the stored vertical `quadraticDealiasing` policy is independent and may be `none`. Registration never reselects modes.
