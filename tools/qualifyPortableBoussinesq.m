function report = qualifyPortableBoussinesq(outputPath,options)
% Qualify Boussinesq continuation and write measured, catalog-linked JSON evidence.
...
schemaIdentifier = "wave-vortex-boussinesq-qualification-v1";
if options.scope=="contracts", schemaIdentifier="wave-vortex-boussinesq-contracts-v1"; end
...
environmentNames = ["WV_STABLE_FORCING_RUNNER","WV_STABLE_FORCING_DUMP","WV_STABLE_FORCING_NATIVE","WV_BOUSS_EVIDENCE_FOLDER","WV_MODAL_DUMP_EXECUTABLE","WV_BOUSS_KERNEL_DUMP","WV_BOUSS_TEST_NATIVE"];
...
if runner == ""
    ...
    runCommand("cmake --build "+shellQuote(build)+" --parallel 4 --target wave-vortex-run WVStableForcingDump WVStratifiedQGFieldDump WVBoussinesqLifecycleProbe WVHydrostaticKernelDump WVStratifiedModalDump WVBoussinesqKernelDump");
    runner = fullfile(build,"wave-vortex-run");
end
for name = ["wave-vortex-run","WVStableForcingDump","WVStratifiedQGFieldDump","WVBoussinesqLifecycleProbe","WVHydrostaticKernelDump","WVStratifiedModalDump","WVBoussinesqKernelDump"]
...
setenv("WV_BOUSS_EVIDENCE_FOLDER",folder);
setenv("WV_BOUSS_KERNEL_DUMP",fullfile(fileparts(runner),"WVBoussinesqKernelDump"));
setenv("WV_BOUSS_TEST_NATIVE",string(double(options.native)));
...
if options.scope=="contracts"
    suite = suite(string({suite.Name})~="TestPortableBoussinesqQualification/longerContinuationMatchesMatlab");
end
...
for row = reshape(catalog.rows(startsWith(string({catalog.rows.configuration}),"boussinesq")),1,[])
    if string(row.acceptance)=="supported"
        names = ["TestPortableBoussinesq/applicableForcingTendenciesMatchMatlab","TestPortableBoussinesq/orderedForcingTendenciesMatchMatlab","TestPortableBoussinesq/applicableForcingContinuationsMatchMatlab"];
        rowProviders = providers;
    else
        names = "TestPortableBoussinesq/incompatibleModelGraphsAreTransactional";
...
cases = readEvidence(folder,"bouss-continuation-*.json");
...
try
    validatePortableBoussinesqQualification(report,repositoryRoot=root);
catch exception
