function validatePortableBoussinesqQualification(report,options)
% Reject incomplete, stale or contradictory Boussinesq qualification evidence.
arguments (Input)
    report (1,1) struct
    options.repositoryRoot (1,1) string = string(fileparts(fileparts(mfilename("fullpath"))))
end
contractsOnly=string(report.schemaIdentifier)=="wave-vortex-boussinesq-contracts-v1";
require((string(report.schemaIdentifier)=="wave-vortex-boussinesq-qualification-v1" || contractsOnly) && report.schemaVersion==1,"Unknown qualification schema.");
...
requiredClasses=["TestPortableBoussinesq","TestPortableBoussinesqQualification","TestPortableStableForcing","TestPortableForcingCompatibility","TestCompiledKernelIntegration","TestStratifiedModalRecord","TestBoussinesqCompiledKernel"];
...
if contractsOnly, expectedNames=expectedNames(expectedNames~="TestPortableBoussinesqQualification/longerContinuationMatchesMatlab"); end
require(isequal(sort(testNames),sort(expectedNames)),"Missing or unexpected qualification tests.");
[catalog,compatibleCatalog] = portableQualificationCatalog(report,"boussinesq",options.repositoryRoot);
require(compatibleCatalog,"Qualification catalog digest or transform slice is stale.");
expected=catalog.rows(startsWith(string({catalog.rows.configuration}),"boussinesq"));
keys=strings(1,0);
for row=reshape(expected,1,[])
    ...
    for provider=rowProviders
        ...
        requiredTests="TestPortableBoussinesq/incompatibleModelGraphsAreTransactional";
        if string(row.acceptance)=="supported"
            requiredTests=["TestPortableBoussinesq/applicableForcingTendenciesMatchMatlab","TestPortableBoussinesq/orderedForcingTendenciesMatchMatlab","TestPortableBoussinesq/applicableForcingContinuationsMatchMatlab"];
        end
...
manifest=jsondecode(fileread(fullfile(options.repositoryRoot,"PortableRuntime","contracts","boussinesq-qualification-cases-v1.json")));
...
for index=1:numel(continuations)
    ...
    for result={row.whole,row.segmented,row.matlabRestart}
        errors=result{1};
    ...
    require(isequal(row.whole.graph,row.segmented.graph) && isequal(row.whole.graph,row.matlabRestart.graph),"Restarted graph identity differs.");
    require(isfinite(row.verticalKineticFraction) && row.verticalKineticFraction>=0 && row.verticalKineticFraction<=1,"Invalid nonhydrostatic regime measurement.");
    if string(definition.regime)=="hydrostatic-limit"
        require(row.verticalKineticFraction<1e-3,"Hydrostatic-limit fixture has excessive vertical kinetic energy.");
    else
        require(row.verticalKineticFraction>.01,"Nonhydrostatic fixture lacks substantive vertical kinetic energy.");
    end
    ...
    intervals=[17 617;17 317;317 617];
    for runIndex=1:numel(runs)
        measured=runs{runIndex};
        require(measured.state.initialTime==intervals(runIndex,1) && measured.state.finalTime==intervals(runIndex,2),"Runtime interval differs from the declared trajectory.");
        require(isfinite(measured.state.stepCount) && measured.state.stepCount>0 && measured.state.rhsEvaluationCount>=measured.state.stepCount,"Missing integration work measurements.");
        require(isfinite(measured.timingSeconds.integrate) && measured.timingSeconds.integrate>0,"Missing runtime timing.");
        require(measured.qualificationScientificStatePreserved,"Persisted wave matrices or opaque source changed.");
...
for index=1:numel(lifecycle)
    ...
    require(string(row.schemaIdentifier)=="wave-vortex-boussinesq-lifecycle-v1","Wrong lifecycle transform.");
...
if ~condition, error("WaveVortexModel:InvalidBoussinesqQualification","%s",message); end
end
