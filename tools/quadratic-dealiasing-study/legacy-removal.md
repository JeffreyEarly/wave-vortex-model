...
The initial hosted R2025b integration passed 1,208/1,212 full tests; all other requested jobs passed. The four full-test failures were old expectations that discovery reports contained only selected modes. A reviewed test-only correction now verifies full discovery, contiguous selected prefixes, and unchanged scientific tolerances. The six affected class methods have passing latest local results; the complete hosted suite is rerun before merge. See the verification ledger for the original failures and targeted corrections.
