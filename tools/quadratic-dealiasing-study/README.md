...
- [`legacy-removal.md`](legacy-removal.md): forthcoming matched before/after evidence for the issue 42 removal of the legacy assessment path.
...
The approved larger-wavenumber figure is archived in the sibling literature project at `literature/ape-apv-free-surface/notes/quadratic-dealiasing-data/retained-modes-larger-k.pdf`, with a PNG rendering beside it. Its inputs are `isolated-wave-counts.csv`, `isolated-wave-summary.json`, and `larger-k-construction-summary.json` in the same directory. The sweep used WVM `ec0c5f23e37a`, released InternalModes beta.7, candidate/reference EVP resolutions 208/312, and the unchanged 65-point WKB grid. Python 3.12 and Matplotlib 3.11.2 generated the figure from those archived records; no larger-k timing or aliasing-error claim is made.

The live construction path uses the new policies. The dedicated legacy survey helpers, obsolete options and metadata, and runnable authoring diagnostic were removed for [#42](https://github.com/JeffreyEarly/internal-modes/issues/42) in WVM cleanup commit `602deb92`; no runtime fallback remains. The matched cleanup evidence will be recorded in [`legacy-removal.md`](legacy-removal.md). Toolbox-free parallel eigensolves remain a separate possible follow-up using the refreshed profile; no parallel speedup is claimed by this study.
