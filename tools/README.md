...
The Benchmarks page contains hand-authored methodology and reproduction instructions. Recorded benchmark data, generated study figures, and execution reports remain outside the repository; documentation generation does not load a benchmark catalog or copy measurements into the website.

The documentation CI job checks the generated source tree when documentation or its generator changes. `validateRenderedWebsite` remains available for a separately requested rendered-site check; rendered output is temporary and is never committed.
