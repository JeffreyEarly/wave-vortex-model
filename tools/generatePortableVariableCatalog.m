...
writeText(headerPath,renderHeader(variables,catalog.contracts));
...
function text = renderHeader(variables,contracts)
lines = [ ...
...
lines = [lines; ...
    ...
    executableCases(variables,contracts); ...
...
catalog.policy = struct(unknownOperation="intentional-incompatibility",customOperation="intentional-incompatibility", ...
    ...
    noMotionSolvers=["dampedLeastSquares","lsqnonlin","fminsearch"],defaultNoMotionSolver="dampedLeastSquares", ...
    shouldUseTrueNoMotionProfileDefault=true,cache="event-scoped-scratch-no-new-persistent-state-sized-cache");
end
...
lines = [lines;"";"## Intentional incompatibilities";""; ...
    ...
    "| MATLAB API | Reason |";"| --- | --- |"];
incompatible = catalog.contracts(string({catalog.contracts.runtimeStatus})=="intentional-incompatibility");
[~,indices] = unique([incompatible.ordinal],"stable");
for row = reshape(incompatible(indices),1,[])
    lines(end+1) = "| `"+string(row.metadata.name)+"` | "+string(row.configurationRestriction)+" |"; %#ok<AGROW>
end
...
function lines = executableCases(variables,contracts)
implemented = unique([contracts(string({contracts.runtimeStatus})=="implemented").ordinal]);
lines = strings(0,1);
for variable = reshape(variables,1,[])
    if ismember(variable.ordinal,implemented)
        lines(end+1,1) = "    case WVPortableVariable::"+variable.cppIdentifier+": return entry;"; %#ok<AGROW>
    end
end
end
