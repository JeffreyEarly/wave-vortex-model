...
inventory=data.inventory; writetable(inventory.interactions,fullfile(outputDirectory,'interactions.csv'));
channels=sourceChannelInventory(); writetable(channels,fullfile(outputDirectory,'channels.csv'));
...
setupSeconds=toc(constructionTimer);
...
for t=indices
    ...
    if mod(t,20)==0, fprintf('Source interactions %d/%d; %d nonzero products; %.1f s.\n',t,height(inventory.interactions),evaluated,toc(timer)); end
end
