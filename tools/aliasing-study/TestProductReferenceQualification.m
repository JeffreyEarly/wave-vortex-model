classdef TestProductReferenceQualification < matlab.unittest.TestCase
    methods (Test)
        function tinyOverlapNeedsExplicitAbsoluteBudget(testCase)
            ...
            q=qualifyProductReferences(context,low,high,1,1,1e-4,1e-10);
            ...
            strict=qualifyProductReferences(context,low,high,1,1,1e-4,0);
        ...
        function rescalingInputsDoesNotChangeQualification(testCase)
            ...
            for j=1:3
                ...
                q=qualifyProductReferences(context,low,high,1,s,1e-4,1e-10);
        ...
        function physicalScaleIncludesSignedEndpointsAndDerivativeFactor(testCase)
            ...
            scale=productReferenceScale(context,a,b,ea,eb);
            ...
            scaled=productReferenceScale(context,7i*a,11*b,7i*ea,11*eb);
            testCase.verifyEqual(scaled,77*scale,RelTol=1e-14)
            volume=productReferenceScale(struct(volumeWeights=[1;2],endpointMetric=[0;0]),a,b,ea,eb);
        ...
        function zeroAndUnmeasurableReferencesAreNotConfused(testCase)
            ...
            q=qualifyProductReferences(context,zero,zero,1,0,1e-4,1e-10);
            testCase.verifyEqual(q.allowanceFraction,0)
            bad=zero; bad.referenceCoefficients={NaN};
            q=qualifyProductReferences(context,bad,zero,1,0,1e-4,1e-10);
