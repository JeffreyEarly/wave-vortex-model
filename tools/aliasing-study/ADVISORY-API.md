...
## QG APV and active-boundary outputs

`prepareQGQuadraticAssessment(source)` and `assessQGQuadraticResolution(prepared)` add a separate optional QG snapshot alongside the wave advisory. They measure the implemented PV-first and residual-endpoint projection, retaining both physical endpoint coordinates and separate fixed-boundary, APV-grid, reference and sampled-product gates. They do not extend the wave source equations to unimplemented nonlinear Boussinesq outputs. `assessQGAssembledTendency(source)` compares a deterministic resolved QG state against direct Fourier convolution and refined vertical references. See the [operator, norm, budget and coverage specification](../../Documentation/Validation/Issue426QGQuadraticAssessment.md); use `runQGQuadraticAssessmentStudy` to reproduce its evidence.
