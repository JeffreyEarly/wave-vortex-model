...
Version 1. Four calibration cases, four withheld cases, and one larger sparse/spot-sample case were declared before scoring. The wider-domain choice and independent fixed counts follow the documented source pilots. No withheld nonlinear errors may be examined while selecting a policy. Changes to the reference apparatus require a recorded numerical reason; they do not authorize retuning a policy using withheld outcomes. Exact historical inventories remain available in Git history and on [issue #400](https://github.com/JeffreyEarly/wave-vortex-model/issues/400).
