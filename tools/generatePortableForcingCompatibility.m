    ...
    for shouldAntialias = [false true]
        ...
        for iForce = 1:numel(supplement.forcings)
            ...
            rows(end+1) = struct(id=configuration.id+"/"+definition.identity,forcing=string(definition.identity), ...
                ...
                implementation=implementation,acceptance="pending",evidence=reshape(evidence,1,[]),barotropicCase=caseName); %#ok<AGROW>
        end
    end
end
matrix = struct(schema="portable-compatibility-matrix-v1",schemaVersion=1,slice="forcing",completion="incomplete", ...
...
try
    ...
    if identity == "WVPseudoTopographicWaveGeneration" && isa(wvt,"WVTransformBarotropicQG") && string(exception.identifier) == "WVPseudoTopographicWaveGeneration:UnsupportedTransform"
        record.applicability = "incompatible";
...
for row = matrix.rows
    ...
    lines(end+1) = "  {"""+row.forcing+""", "+cppBoolean(startsWith(row.configuration,"barotropic"))+", "+ ...
