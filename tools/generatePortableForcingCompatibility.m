...
for family = ["constant-hydrostatic","constant-nonhydrostatic","barotropic","stratified-qg","hydrostatic","boussinesq"]
    for shouldAntialias = [false true]
        ...
        isHydrostatic = ~ismember(family,["constant-nonhydrostatic","boussinesq"]);
        ...
        if family == "hydrostatic", identity = "WVTransformHydrostatic"; end
        if family == "boussinesq", identity = "WVTransformBoussinesq"; end
        ...
        for iForce = 1:numel(supplement.forcings)
            ...
            if family == "boussinesq"
                wvt = WVTransformBoussinesq([17000 11000 1000],[8 6 9],Nj=4,N2Function=@(z) 1e-4*exp(z/700),shouldAntialias=shouldAntialias);
                available = definition.boussinesqFactory;
                evidence = string(definition.boussinesqEvidence);
                caseName = "";
            elseif family == "hydrostatic"
                wvt = WVTransformHydrostatic([17000 11000 1000],[8 6 9],Nj=4,N2Function=@(z) 1e-4*exp(z/700),shouldAntialias=shouldAntialias);
                available = definition.hydrostaticFactory;
                evidence = string(definition.hydrostaticEvidence);
                caseName = "";
            elseif isStratifiedQG
            ...
            if string(supplement.qualification) == "baseline-v1"
                if scientific.applicability == "incompatible"
                    ...
                    if family == "hydrostatic", evidence = ["hydro-rejections","matlab-attachment"]; end
                    if family == "boussinesq", evidence = ["bouss-rejections","matlab-attachment"]; end
                elseif available
...
lines = ["// Generated by tools/generatePortableForcingCompatibility.m. Do not edit."; ...
    ...
    "struct ForcingCompatibilityRow { const char *identity; bool barotropic; bool stratifiedQG; bool hydrostatic; bool boussinesq; bool antialias; bool applicable; bool factoryAvailable; std::uint32_t version; std::uint8_t priority; const char *stage; };"; ...
...
for row = matrix.rows
    ...
    lines(end+1) = "  {"""+row.forcing+""", "+cppBoolean(startsWith(row.configuration,"barotropic"))+", "+cppBoolean(startsWith(row.configuration,"stratified-qg"))+", "+cppBoolean(startsWith(row.configuration,"hydrostatic"))+", "+cppBoolean(startsWith(row.configuration,"boussinesq"))+", "+cppBoolean(endsWith(row.configuration,"aa1"))+", "+ ...
