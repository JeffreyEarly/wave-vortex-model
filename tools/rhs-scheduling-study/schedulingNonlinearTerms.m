function terms = schedulingNonlinearTerms(hatted,pressure,xi,D,f,rho0,N2,buoyancyRemainder,derivative,pressureGradient,sourceOnly)
...
if isempty(fieldnames(pressureGradient))
    pressureX = derivative.x(pressure);
    pressureY = derivative.y(pressure);
    pressureXi = derivative.xi(pressure);
else
    pressureX=pressureGradient.x; pressureY=pressureGradient.y; pressureXi=pressureGradient.xi;
end
