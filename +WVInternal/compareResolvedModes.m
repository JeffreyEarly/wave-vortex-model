function [report,errors] = compareResolvedModes(basis,reference,N2,nQuadrature)
...
identity=struct(family=string(basis.evp.modeFamily),columnLabels=string(basis.modeNumber),normalization=string(basis.normalization),zDomain=basis.zDomain);
...
errors=inf(numel(basis.modeNumber),1);
for j=1:numel(errors)
    ...
    function value=prepare(source)
        ...
        value=struct(identity=identity,values=struct(F=fields.F,G=fields.G),derivatives=struct(F=fields.dF,G=fields.dG),equivalentDepths=source.h,provenance=struct(solverClass="IMSolverSpectral",coordinateKind=string(source.solver.coordinateKind),nEVP=source.solver.nEVP,quadratureCoordinateKind="wkb",quadratureCount=nQuadrature,source="actual construction basis"));
    end
end
