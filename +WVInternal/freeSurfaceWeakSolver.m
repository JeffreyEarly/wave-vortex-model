...
solver = struct(solve=@solve,layout=layout,boundary=boundary,referenceMass=reference);

    function [state,report] = solve(rhs,target,metric,options)
        arguments (Input)
            rhs (1,1) struct
        ...
        p = z;
        iteration = 0;
        while sqrt(max(0,rz))>options.tolerance*scale && iteration<options.maxIterations
            hp = applyH(p);
            curvature = real(p.'*hp);
        ...
        report = struct(iterations=iteration,relativeResidual=relativeResidual,absoluteResidual=absoluteResidual,constraintResidual=constraintResidual,multiplier=multiplier);
    end
    ...
    function value = dualNorm(vector)
        value = sqrt(max(0,real(vector.'*applyB(vector))));
    end
end
