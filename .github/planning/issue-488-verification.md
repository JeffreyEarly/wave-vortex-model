...
- Actual forward/restart execution passes all six configurations with both providers, exercising runner and controlled-stop probe through the new guard. Focused unit tests cover the shared validator for both role paths; the lifecycle run establishes actual selector coverage. Independent review found no blocker.
- Collected fresh source-linked receipts. The changed test-source fingerprint required canonical compatibility-matrix regeneration; generated documentation was unchanged. All 10 forward catalog tests and seven compatibility tests pass. No production MATLAB or C++ implementation changed.
