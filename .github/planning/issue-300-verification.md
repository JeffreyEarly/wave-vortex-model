...
- Final observer qualification adds horizontal-only advection on 3D tracer grids and explicitly exercises fully 3D particles as well as fixed-depth drifters. The affected six MATLAB methods pass again with reference/native and sanitized reference execution (`/private/tmp/wvm-300-xy-parity.log`, `wvm-300-xy-asan-parity.log`). Hydrostatic C++ boundary and GCC checks pass again; the scalar-advection API change does not affect the existing transform algorithms. Code Analyzer remains clean for the updated test.
