...
- The first Linux sanitizer qualification exposed a test-harness defect: allocation interposition omitted `nothrow new`, used by libstdc++ `stable_sort`, while its delete override called `free`. Add scalar/array/aligned nothrow allocation and matching deletion overloads rather than disabling ASan mismatch checks. A direct regression reproduces the original mismatch on macOS with `alloc_dealloc_mismatch=1`, then passes with the fix; it is registered in kernel and runtime CTest projects. Instrumented modal-record and SQG-kernel contracts also pass with mismatch checking enabled.
- The initial Linux release run passed its SQG and stable-forcing methods but reached the 20-minute job limit during catalog validation. The largest reference lifecycle grid alone took nearly seven minutes on that runner. The focused workflow now allows 35 minutes; fixture coverage and numerical criteria remain unchanged.
