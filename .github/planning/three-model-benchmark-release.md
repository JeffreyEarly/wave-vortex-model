...
- Retain deterministic seed 4001, use the qualified common GM(0.5) energy level (see full-grid correction below), and first-baroclinic red geostrophic spectrum with maximum horizontal geostrophic speed 0.15 m/s. Same initialization recipe across families; one saved state shared across interfaces within each model/workload.
...
## Combined qualified-source handoff

PR #509 merged as `f70c8908`, with the exact tree of qualified head `8b40ca8f`. This benchmark series is rebased onto that merge and now also carries the final #307/#310 parity decision and its metadata/schema/provenance validation. The two independently verified metadata/harness increments share one final hosted submission; no C++ or MATLAB scientific implementation changed. The 51 benchmark/presentation checks and 7 MATLAB plus 6 Python readiness checks remain applicable to the unchanged combined files. Full-size measurements, new timing publication, and the v4.4.0 release remain pending under #501; no new timing values are fabricated or substituted.

The combined source at `10448867` passed a fresh all-interface smoke campaign: 18 processes covering all three models, both workloads, and MATLAB builtin / MATLAB compiled / standalone C++. At `[32 32 17]` over 256 s, all complete-output and integration-control comparisons pass, with maximum relative error below 4.5e-15. These records are explicitly publication-ineligible and supply no performance claim. The compact receipt is `.github/ci-evidence/issue-501-benchmark-preparation/all-interface-smoke.json`; raw records and driver are preserved in `../wave-vortex-model-benchmark-artifacts/three-model-smoke-10448867`. The required full-size pilot and 54-run release campaign remain pending.

## Approved cleanup and hosted assertion correction

September 13: the owner approved the exact 80-file cleanup manifest. All 80 SHA-256 values and successful comparison records were verified before deletion; only the listed generated `.bin` payloads were removed (16,804,540,800 bytes). The metadata archive and deletion receipt are `../wave-vortex-model-benchmark-artifacts/wvm358-approved-payload-cleanup-20260913.tar.gz` and its adjacent `.json` receipt. Free space after cleanup was 24,117,944,320 bytes. Earlier storage-hold entries above are historical.

PR #510 initial hosted run 34750696512 passed native, package, and scientific tests, but both MATLAB releases failed the same `TestUserDocumentation/compiledExecutionNavigationAndRoutesAreStable` method because it still asserted superseded benchmark prose and the deliberately removed appended optimization section. The narrow correction updates these documentation assertions. Pre-pilot inspection also found that the fixture storage budget still counted only two variable-model interfaces; all current matched models must budget three. No scientific algorithm or tolerance changed.

The corrected documentation method passes locally (1/1, MATLAB R2025b); Code Analyzer reports only two pre-existing informational extra-comma findings in the harness. Logs: `/private/tmp/wvm503/pr510-correction.log`, `pr510-documentation-correction-tests.csv`, and `pr510-correction-analyzer.csv`. Existing passing scientific and documentation-generation evidence remains applicable; this change does not alter generated pages. Full-size pilot will exercise the corrected three-interface retention estimate.

## Full-grid scientific preflight and common recipe correction

The full-grid constant-stratification GM(1) pilot at source `6274682b` passed all six interface/workload runs and complete-output/integration-control comparisons in 309.864 seconds total wall time. These 256-second, single-repeat results are publication-ineligible. The host is Apple M4 Max with 128 GiB RAM, MATLAB R2025b Update 4 and 16 requested threads; publication must use actual recorded hardware labels.

The Hydrostatic GM(1) full-grid initial condition failed the unchanged density-admissibility check before any worker timing. Independent full-grid diagnostics constructed both exponential models, initialized the same seed and first-baroclinic 0.15 m/s geostrophic component, and tested wave energies 0.25, 0.5, 0.75 and 1. Both models pass at 0.25 and 0.5 and fail at 0.75 and 1. At GM(1), the combined density minima fall 0.00561045 kg/m³ (Hydrostatic) and 0.00460247 kg/m³ (Boussinesq) below the no-motion minimum. Boussinesq construction took 157.463 seconds, outside integration timing.

The new matched release campaign therefore uses the common GM energy level 0.5 for all three models, retaining every other domain, grid, stratification, seed, geostrophic speed, integration and output setting. This pre-freeze correction makes the prescribed state admissible and keeps interfaces matched; it does not alter MATLAB scientific code, density validation or historical GM(1) studies. The current recipe has a distinct provenance identity, and its final combined state is checked before fixture export. Earlier GM(1) plan and pilot entries remain historical evidence, not qualified current-publication records.

Ten source-bound pilot/diagnostic records, including the original failure and both amplitude sweeps, are archived with SHA-256 manifest at `../wave-vortex-model-benchmark-artifacts/three-model-full-grid-preflight-6274682b`. The original 80-payload cleanup evidence remains separately archived. No full 54-process campaign or v4.4.0 publication has occurred at this stage.

Final recipe/release preparation checks: all 53 focused benchmark, website and navigation methods pass; Code Analyzer reports informational findings only; documentation build/check reports 2,052 files and 4,197 routes with zero failures or generated differences. Logs are `/private/tmp/wvm503/v44-recipe-release-contract.log`, `v44-recipe-release-contract-tests.csv`, and `v44-recipe-release-analyzer.csv`. Independent review confirms matched-only final-density validation and preserved legacy GM(1) defaults/artifacts. No passing native or adapter scientific suite was repeated for this authoring-only correction.

## Native policy selection found by the full-grid pilot

PR #510 merged as `5fc15da3` with the exact tree of qualified `6274682b`; #307 and #310 are closed. The GM(0.5) Hydrostatic pilot at `1a29f140` passed all six numerical/interface/workload comparisons in 328.382 seconds, but its timing exposed a benchmark configuration mismatch. The MATLAB compiled endpoint took 8.90 seconds, while standalone took 27.53 seconds for the same ten steps and 130 RHS evaluations. These single-repeat values diagnose the harness and are not publication results.

The harness always passed standalone `--threads 16`. For variable kernels, an explicit FFT-thread request greater than one deliberately selects `established-explicit-fft-threads`, retaining Accelerate matrices but using the established full-FFT/interleaved schedule. The compiled MATLAB adapter selects the qualified `compact-native-accelerate` policy, with one FFTW thread per transform and parallel compact work. CMake enabling Accelerate and compact policy does not override the explicit CLI choice. The prior provider record also replaced effective thread details with the configured host budget, hiding this distinction.

Correct the matched variable benchmark to select the normal optimized runner policy, retain legacy and constant behavior, and capture actual selected policy/effective FFT threads separately from the host thread budget. Require the intended compact Accelerate policy in current matched publication records. No production policy or scientific algorithm is changed. The original numerically passing but policy-mismatched pilot remains archived with its hashes at `../wave-vortex-model-benchmark-artifacts/three-model-policy-preflight-1a29f140`.

Coordinator verification corrected an initial agent diagnosis: at the exact pilot source `1a29f140`, Accelerate selection precedes the explicit-thread return. Both routes use Accelerate; the mismatch concerns FFT/internal-versus-external parallelism and the established versus compact fused/tiled schedules. No scalar-backend regression is established or claimed.

Policy/provenance correction validation: 58 focused benchmark, website and navigation tests pass: 57 selected together, followed by the separate scalar-matrix-policy rejection method. Coverage includes explicit-thread schedule, wrong matrix backend, differing worker policy, effective-thread mismatch, and historical contract negatives. Code Analyzer reports informational findings; documentation check passes (2,052 files, 4,197 routes, no differences). Logs: `/private/tmp/wvm503/v44-policy-contract.log`, `v44-policy-contract-tests.csv`, and `v44-policy-analyzer.csv`. Actual full-grid producer records remain the next acceptance step.

## Final full-grid policy pilots and worker teardown

Frozen `0c8347c1f07ea049b426bfd21c1856d7471ad2ff` passes all 18 full-grid pilot processes (three models, three interfaces, two workloads) and numerical, output, integration-control and actual-policy gates. The 256-second, one-repeat pilots are deliberately ineligible for publication. Complete campaign wall times were 284.089 seconds for Hydrostatic, 915.572 seconds for Boussinesq and 349.746 seconds for constant stratification; model construction is outside reported integration timing. Compact receipts and logs are archived with SHA-256 manifest at `../wave-vortex-model-benchmark-artifacts/three-model-qualified-pilots-0c8347c1`.

The constant compiled composite worker emitted a teardown warning after its successful scientific work. Inspection found that the worker restored the prior MATLAB path while its local transform remained alive, allowing the destructor to resolve an older same-name cached MEX. Both success and exception paths now destroy the model and then independently destroy the transform before restoring the path. This authoring-only cleanup occurs outside integration timing; all production MATLAB/C++ sources, scientific inputs, algorithms and numerical tolerances remain identical to the qualified pilot source. The successful worker stdout was transient and is unavailable after harness cleanup; the diagnosis and focused reproduction are retained separately.

Focused cleanup qualification passes for successful fixed-RK4 continuation and an intentional unsupported-integrator failure after compiled construction, each in a fresh MATLAB process with an older same-name MEX on the prior path and cleanup warnings promoted to errors. A preliminary serial-process driver encountered the backend's fail-closed capability check before its second worker; the actual fresh-process lifecycle passes. Receipt: `.github/ci-evidence/issue-501-benchmark-preparation/worker-cleanup.json`. Two unused catch assignments identified by Code Analyzer were removed without changing the tested destruction order; the existing unused-input warning remains. Production source, native build inputs, timing boundaries and prior numerical qualifications are unchanged.
