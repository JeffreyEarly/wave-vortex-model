...
Hosted PR #509 qualification found GCC misleading-indentation errors at two constant raw horizontal return sites and a Clang/Linux missing `<stdexcept>` include in the forcing workspace. The correction only splits existing statements onto separate lines and supplies the direct standard header; no expressions, control flow, or arithmetic change. The affected native primitive/forcing tests pass (3/3, 1.18 s). Earlier numerical receipts retain their original source identities and remain numerical evidence by this explicit equivalence; the corrected source receives fresh hosted build qualification.

The disposable MPM package installed successfully with the six released dependencies. Its compiled build exposed a pre-existing packaging limitation: MPM installs a read-only package, while the builder places generated cache/module files under that package. Installed compiled qualification remains pending a writable user-cache correction; no permission changes are made to installed package sources.

Installed-cache correction: all generated provider, staging, state and MEX files now live under the macOS user cache, keyed by canonical package location, architecture and MATLAB release. MATLAB `prefdir` is deliberately not used for compiled sources because its usual Application Support path contains whitespace rejected by FFTW's configure script. Exact-path module activation rejects loaded foreign modules and working-directory shadows before invocation; inspection restores path/module state. Earlier generated authoring MEX bytes are preserved in their ignored cache archive with SHA-256 unchanged. The 8 cheap cache/support/failure tests pass and the 187-file production analyzer reports zero blockers. Native build/rollback, all-family public checks and fresh read-only installation qualification remain pending at this source freeze.

External-cache follow-up: native provider/MEX build, transaction rollback, all public adapter checks and source/binary identity checks pass. The one initial foreign-module test error was a test call omitting moduleInfo's required output; its corrected invocation passes and verifies that inspection rejects and preserves the foreign loaded module. The focused five-method provenance/analyzer/loader rerun passes. Hosted MATLAB also exposed a stale constant-kernel current-source fingerprint after the portability-only formatting fix and R2026a wording for the previously reviewed version() Code Analyzer false positive; the current key and narrow wording recognition are corrected without changing production sources. Documentation build/check passes with 2,047 files and 4,187 routes. Read-only installed compilation and representative profiling remain pending.
