...
| --- | --- | --- | --- |
| [#344](https://github.com/JeffreyEarly/wave-vortex-model/issues/344) transform/state | `WVTransformFreeSurfaceQG` exposes and persists the three canonical families; four endpoint configurations, mutation, cache invalidation, MDA constraints, direct construction, snapshot/output restart, and inactive-family omission have focused tests. #343 now selects the required backing. | Automatic representative construction is not yet reliable because #344 delegates common-`Nj` selection to #346. Preserve legacy rigid-lid regressions in the final series gate. | Implementation substantially satisfies #344, but the issue should remain dependent on #346. |
...
| [#346](https://github.com/JeffreyEarly/wave-vortex-model/issues/346) grid/common `Nj` | Constant stratification selects and persists a common prefix; explicit values above the certified maximum are rejected; the `0.1` product-aliasing tolerance and compact diagnostics are exposed. | Resolve reproducibility for variable stratification and the automatic MDA refit. A reduced one-endpoint exponential profile can produce `IMBasisSet:NoAcceptableDiscreteTransformPrefix`; the representative constant two-endpoint automatic path can produce `IMBasisSet:QuadratureWeightFitFailed` when refitting MDA at the APV-limited count. Add an MDA-limits-first case, independent over-resolved product experiments, threshold provenance, depth-closure checks, and maximum-`kh` zero-APV residual coverage. | Not accepted; this is the principal scientific blocker after #343. |
