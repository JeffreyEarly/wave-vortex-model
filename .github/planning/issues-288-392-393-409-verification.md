...
Required run `34308166199` passed all45 Release and all45 ASan/UBSan C++ contracts, but both MATLAB releases correctly detected stale generated energy-operation provenance. Exactly44 `operationExpression` strings changed from the inline lambda to `@(wvt)maskedEnergy(wvt,mask)` in the catalog JSON and generated contracts header. Regeneration preserved every other JSON value: all76 identities,666 contracts, metadata, dependencies, classifications and23 legacy entries; the main catalog header and VARIABLES.md are unchanged. All six MATLAB catalog methods and the rebuilt native catalog consumer test passed. These strings are provenance with no runtime evaluator; no numerical or performance rerun is warranted. The next required run validates the regenerated files on both releases.
