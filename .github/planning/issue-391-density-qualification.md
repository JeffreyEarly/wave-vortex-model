...
## Approved production adoption

The owner approved actual `rho_nm` by default on 2026-09-09. The default solver is now the qualified damped implementation. Current extrema bound default nonuniform fits; stable rest remains exact. Displacement and APE now share the monotone cubic profile, and APV uses that displacement. Explicit false selects the original profile consistently in both displacement and APE. Existing file schemas remain valid, with the new default applied on load.

`final-production-day3000.json` and `final-production-day3250.json` qualify the actual public default pipeline from the restored snapshots, without injected profiles or flag overrides. All seven checks per snapshot pass: actual default, damped solver qualification, strict profile ordering, material-height bounds, density closure, finite/nonnegative APE, and finite APV. Final maximum normalized moment residuals are 4.64e-11 and 2.12e-11, density closure is zero at stored double precision, and mean displacement is -0.009446 m and +0.003645 m. Combined rho_nm/displacement/APE/APV measurements are 0.703 s and 5.710 s, excluding load and qualification checks. These are individual local observations, not a controlled speed guarantee. Reports record exact source hashes, including the uncommitted production edits on top of their parent revision.

The prior default-selected high-order spline and the new shared cubic need not yield identical diagnostics. Saved historical aggregate comparisons remain labeled as the explicitly selected actual-profile legacy path, not the former default-false approximation. No historical full-field APV comparison was captured.

The continuous overturning experiment additionally initializes real coefficients from velocity/displacement fields and evaluates the complete default pipeline on 16/32/64 grids. It records spectral projection error separately from continuum distribution error. The independent material-height reference establishes convergence, rather than assuming sampled twists exactly preserve the discrete parcel distribution. Its final source-bound report is `final-continuous-twist.json`.
