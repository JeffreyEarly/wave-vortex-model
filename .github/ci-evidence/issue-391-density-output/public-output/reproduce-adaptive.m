...
suite=suite(contains(string({suite.Name}),"adaptiveDensityOutputAndRestartPreserveIntegration"));
assert(numel(suite)==1);
results=run(suite); disp(table(results));
stream=fopen('/private/tmp/wvm391-density-adaptive-output-results.json','w');
