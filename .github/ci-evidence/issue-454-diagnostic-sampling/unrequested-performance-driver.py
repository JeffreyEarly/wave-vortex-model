...
for p in profiles:
 for index in range(-2,8):
  ...
  comparison=helper.compare_graph(a,b,0,0);helper.save(OUT/(p['id']+f'-{index}-comparison.json'),comparison);assert comparison['passed'],comparison['differences']
  if index==0:
...
helper.save(OUT/'summary.json',{'passed':passed,'profiles':summary,'equalProfileBootstrap95':ci,'allExactComparisonsPassed':True,'measuredPairs':16,'warmupPairs':4,'postflightUnchanged':True})
for k,v in executables.items(): assert helper.digest(v)==hashes[k]
assert source_hashes=={p:helper.digest(ROOT/p) for p in source_paths}
assert input_hashes=={str(p):helper.digest(p) for p in inputs}
