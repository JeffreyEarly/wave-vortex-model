...
WVKernelStatus WVTransformHydrostaticKernel::prepareRealVerticalCalculus(MatrixBackendFactory factory) {
    // Hydrostatic F0/G0 matrices are independent of horizontal mode. Preserve
    // the general path for a source providing genuinely different matrices.
    for (const auto& operation:vertical_) if (operation->uniqueMatrixCount()!=1) return WVKernelStatus::ok();
    const auto& g=geometry();
    ...
    for (std::size_t operation=0;operation<5;++operation) {
        ...
        status=kernel_detail::applyStratifiedVerticalCalculus(g,g.Nz,inputIsF,order,integral,
            ...
            [&](std::size_t op,WVComplexInput input,WVComplexOutput output) {
                return vertical_[op]->execute(*verticalWorkspace_[op],input,output);
            });
        if (!status) return status;
...
WVKernelStatus WVTransformHydrostaticKernel::verticalCalculus(const double* values,WVHydrostaticFamily family,
    ...
    if (realVerticalBackend_ && values!=result) {
        ...
        // A physical volume already has layout [horizontal columns,Nz].
        // Multiply on the right by D^T without permuting either full volume.
        ...
        // These two existing scratch slots are disjoint from advection's x/y/z
        // derivatives (slots 0:2). No additional physical-volume buffers.
    ...
    return kernel_detail::applyStratifiedVerticalCalculus(g,columns,family==WVHydrostaticFamily::F,
...
WVKernelStatus WVTransformHydrostaticKernel::applyVerticalCalculus(WVRealConstView a,bool inputIsF,
    ...
    ActiveCall guard(active_); if (!guard.entered) return reentrant();
    return verticalCalculus(a.data,inputIsF ? WVHydrostaticFamily::F : WVHydrostaticFamily::G,
...
WVKernelStatus WVTransformHydrostaticKernel::advectScalarWithAdvectionFields(WVRealVolumeConstView a,WVRealFieldBundleConstView fields,bool antialias,WVRealVolumeView b,bool xyOnly) {
    ...
    if (!xyOnly) { s=verticalCalculus(a.data,WVHydrostaticFamily::F,1,false,real_.data()+2*R_,R_/geometry().Nz,false); if (!s) return s; }
