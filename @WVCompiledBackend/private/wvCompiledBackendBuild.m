...
runtimeSources = fullfile(constants.runtimeSourceDirectory,[ ...
    ...
    "WVForcingEngine.cpp"; ...
    "WVBarotropicQGForcingEngine.cpp"; ...
    "WVStratifiedQGForcingEngine.cpp"; ...
    "WVHydrostaticForcingEngine.cpp"; ...
    "WVBoussinesqForcingEngine.cpp"; ...
    "WVBarotropicQGIntegrationSystem.cpp"; ...
    "WVStratifiedQGIntegrationSystem.cpp"; ...
    "WVHydrostaticIntegrationSystem.cpp"; ...
    "WVBoussinesqIntegrationSystem.cpp"; ...
    ...
    "WVBarotropicQGFieldEvaluationAdapter.cpp"; ...
    "WVStratifiedFieldEvaluationAdapter.cpp"; ...
    "WVDiagnosticFieldPlan.cpp"; ...
    "WVDensityEventEvaluation.cpp"; ...
    "WVNoMotionProfile.cpp"; ...
    "WVNoMotionProfileRecovery.cpp"; ...
    "WVNoMotionDensityMoments.cpp"; ...
    "WVConstantStratificationIntegrationSystem.cpp"; ...
    "WVModelTransformAdapters.cpp"; ...
    "WVModel.cpp"]);
runtimeHeaders = fullfile(constants.runtimeIncludeDirectory,"WaveVortexRuntime",[ ...
    "WVBarotropicQGForcingEngine.hpp"; ...
    "WVCheckpointReader.hpp"; ...
    "WVExtensionCatalog.hpp"; ...
    "WVFieldEvaluationService.hpp"; ...
    "WVForcing.hpp"; ...
    "WVForcingContracts.hpp"; ...
    "WVModel.hpp"; ...
    "WVModelOutputConfiguration.hpp"; ...
    "WVObservation.hpp"; ...
    "WVObserverContracts.hpp"; ...
    "WVObserverOutputProvider.hpp"; ...
    "WVObservingSystem.hpp"; ...
    "WVOutputSchedule.hpp"; ...
    "WVPortableImplementationContract.hpp"; ...
    "WVPortableTypedRecord.hpp"]);
runtimePrivateHeaders = fullfile(constants.runtimeSourceDirectory,[ ...
    "WVBarotropicQGFieldEvaluationAdapter.hpp"; ...
    "WVLegacyObserverCompatibility.hpp"; ...
    "WVModelTransformAdapters.hpp"; ...
    "WVModelInternalAccess.hpp"]);
coreSources = fullfile(constants.coreSourceDirectory,[ ...
    "WVKernelTypes.cpp"; ...
...
for iCase = 1:2
    ...
    expectedPointwiseWorkers = 1;
    if constants.compactConstantDefault
        expectedSchedule = "retained-compact-streamed-target-three-channel-v1";
        expectedRows = wvt.Nkl;
        expectedHorizontalWorkers = min(constants.horizontalOuterWorkers,3*wvt.Nz);
        expectedPointwiseWorkers = constants.pointwiseWorkers;
    end
    metadataPassed = string(metrics.nonlinearFluxSchedule)==expectedSchedule && string(estimate.nonlinearFluxSchedule)==expectedSchedule && metrics.verticalExecutionRowCount==expectedRows && estimate.verticalExecutionRowCount==expectedRows && string(metrics.planCountMeaning)=="logical-prepared-operation-slots" && metrics.executionScheduleVersion==1 && string(metrics.workerPolicyIdentifier)==constants.executionPolicyIdentifier && metrics.retainedHorizontalWorkerCount==expectedHorizontalWorkers && metrics.pointwiseWorkerCount==expectedPointwiseWorkers;
    halfValues = expectedRows*wvt.Nz*4;
    fullHalfValues = (floor(wvt.Nx/2)+1)*wvt.Ny*wvt.Nz;
    scalarHalfValues = halfValues;
    if constants.compactConstantDefault, scalarHalfValues = max(halfValues,fullHalfValues); end
    metadataPassed = metadataPassed && estimate.halfSpectrumScratchCapacityBytes==16*halfValues && estimate.configuredScalarScratchCapacityBytes==16*scalarHalfValues+48*wvt.Nx*wvt.Ny*wvt.Nz && metrics.phaseValueBytes==16*wvt.Nj*wvt.Nkl;
    if maximumRelativeError > 1e-12 || metrics.planCount ~= 17 || metrics.contractVersion ~= constants.contractVersion || ~lifecyclePassed || ~estimatePassed || ~metadataPassed
        error("WaveVortexModel:CompiledBackendSelfTest","The %s self-test failed: error %.3g, plans %d, contract %d, lifecycle %d, estimate %d, metadata %d.",definition.id,maximumRelativeError,metrics.planCount,metrics.contractVersion,lifecyclePassed,estimatePassed,metadataPassed);
    end
    ...
    estimateCases(iCase) = struct("id",definition.id,"Nxyz",[estimate.Nx estimate.Ny estimate.Nz],"isHydrostatic",definition.isHydrostatic,"persistentBytes",metrics.persistentBytes,"stateInputBytes",metrics.stateInputBytes,"fluxOutputBytes",metrics.fluxOutputBytes,"knownMaximumLiveOwnedBytes",metrics.knownMaximumLiveOwnedBytes,"planBytes",metrics.planBytes,"planMemoryAccounting",string(metrics.planMemoryAccounting));
end
...
storageEstimates = struct("status","measured-after-kernel-construction","formulaSource","Application-owned kernel, provider, descriptor, plan-wrapper, and bounded scratch storage; FFTW plan internals remain opaque","cases",estimateCases);
end
