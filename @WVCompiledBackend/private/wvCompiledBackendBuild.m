...
requiredSources = [string(gateway);string(engine);fullfile(constants.adapterDirectory,"WVNativeFFTWEngine.hpp");fullfile(constants.coreSourceDirectory,"WVKernelTypes.cpp");fullfile(constants.coreSourceDirectory,"WVTransformConstantStratificationKernel.cpp")];
...
compilerFlags = "CXXFLAGS=$CXXFLAGS -std=c++17 -pthread -O3 -mcpu=native -mmacosx-version-min="+constants.deploymentTarget+" -DWV_KERNEL_NATIVE_OPTIMIZATION=1 -DWV_KERNEL_COEFFICIENT_WORKERS=2";
...
mex("-R2018a",compilerFlags,gateway,fullfile(constants.coreSourceDirectory,"WVKernelTypes.cpp"),fullfile(constants.coreSourceDirectory,"WVTransformConstantStratificationKernel.cpp"),engine,"-I"+constants.coreIncludeDirectory,"-I"+constants.adapterDirectory,"-I"+fullfile(constants.installDirectory,"include"),linkerFlags,libraries.thread.path,libraries.base.path,"-outdir",constants.stageDirectory,"-output",constants.moduleName);
