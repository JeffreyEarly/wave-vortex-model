...
% Create one operation sharing reconstruction across requested thermal fields.
%
...
% - Parameter variableName: requested thermal field names, supplied as separate arguments
...
if isempty(names) || length(unique(names)) ~= length(names) || any(~ismember(names,string(self.namesOfTransformVariables())))
    error('WVTransform:UnknownVariable','Request distinct thermal fields listed by namesOfTransformVariables.')
end
...
for name = names
    ...
    switch name
        ...
        case "buoyancy", units = 'm s-2'; description = 'physical buoyancy';
        case "endpointAnomalies", units = 'm'; description = 'surface and bottom displacement'; dimensions = {'x','y','activeEndpoint'};
    ...
    annotation.isVariableWithLinearTimeStep = false;
    annotation.isVariableWithNonlinearTimeStep = true;
    annotation.isDependentOnApAmA0 = true;
    annotations(end+1) = annotation; %#ok<AGROW> At most twelve field annotations.
end
operationName = "thermalFields_"+join(names,"_")+suffix;
