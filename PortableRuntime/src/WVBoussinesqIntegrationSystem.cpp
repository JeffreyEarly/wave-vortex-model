...
WVKernelStatus WVBoussinesqIntegrationSystem::create(
...
WVKernelStatus WVBoussinesqIntegrationSystem::createImpl(
    std::shared_ptr<const WVStratifiedModalSource> source,
    const WVFrozenForcingSchedule &schedule,
    const WVPortableObserverDescriptor *descriptor,
    std::shared_ptr<const WVExtensionCatalog> catalog,
  ...
  if (descriptor != nullptr && descriptor->catalog() != catalog)
    return {WVKernelStatusCode::invalidConfiguration,
            ...
            "extension catalog."};
  try {
    if (!source) return invalid("Boussinesq integration requires a scientific source.");
    const auto& configuration=source->geometry();
    auto candidate =
        std::unique_ptr<WVBoussinesqIntegrationSystem>(
            new WVBoussinesqIntegrationSystem());
    auto status = WVBoussinesqForcingEngine::create(
        std::move(source), schedule, std::move(catalog), std::move(engine),
        candidate->forcing_, services);
    if (!status)
      return status;
    const auto coefficientShape =
        candidate->forcing_->kernel().spectralShape();
    ...
    integrationBinder.advectedScalar =
      ...
      if (dimensions != std::vector<std::size_t>(
        ...
        return {WVKernelStatusCode::invalidShape,
                "A Boussinesq tracer must have shape [Nx,Ny,Nz]."};
      ++ownerCounts[block];
      WVTracer tracer;
    ...
    integrationBinder.advectedPositions =
      ...
      if (observer.isXYOnly && observer.z.size() != observer.x.size())
        return invalid("Boussinesq XY particles require one fixed "
    ...
    return {WVKernelStatusCode::allocationFailure,
            "Unable to allocate the Boussinesq particle system."};
  }
}

WVBoussinesqIntegrationSystem::~WVBoussinesqIntegrationSystem() =
    default;

WVKernelStatus
WVBoussinesqIntegrationSystem::evaluateRightHandSide(
  ...
  if (executing_)
    return {WVKernelStatusCode::reentrantExecution,
            "The Boussinesq integration system is not reentrant."};
  ...
  detail::WVScopedStateEvaluation<WVBoussinesqForcingEngine> evaluation(*forcing_,state.waveVortex);
  ...
  status=forcing_->nonlinearFlux(state.waveVortex,rightHandSide.waveVortex);
  if (status && needsAdvectionContext) {
    status=forcing_->physicalFields(state.waveVortex,advection);
    advection.shape.fourth=3;
  }
  if (!status)
    return status;
  ...
  const auto spatial = forcing_->kernel().spatialShape();
...
WVBoussinesqIntegrationSystem::enforceStateConstraints(
...
WVKernelStatus
WVBoussinesqIntegrationSystem::initializeParticleState(
...
WVKernelStatus WVBoussinesqIntegrationSystem::createErrorPolicy(
...
WVKernelStatus
WVBoussinesqIntegrationSystem::evaluateFixedTimeStepCandidates(
  ...
  const auto &configuration = kernel().geometry();
  const auto spatial=kernel().spatialShape(); const auto fieldElements=spatial.elementCount();
  WVRealFieldBundleConstView fields;
  status=forcing_->physicalFields(state.waveVortex,fields); if (!status) return status;
  {
    WVFixedTimeStepCandidates result;
    result.transientWorkspaceMaximumLiveBytes = 0;
    ...
    for (std::size_t i=0;i<kernel().factors().size();++i) if (configuration.j[i%configuration.Nj]>0)
      result.highestActiveWaveFrequency=std::max(result.highestActiveWaveFrequency,kernel().factors()[i].omega);
...
std::size_t
WVBoussinesqIntegrationSystem::persistentBytes() const noexcept {
