...
WVCheckpointStatus WVModelOutputNetCDFSink::restoreState(
  ...
  try {
    ...
    if (restoredCheckpoint.transformKind == WVPersistedTransformKind::stratifiedQG || (restoredCheckpoint.transformKind == WVPersistedTransformKind::hydrostatic || restoredCheckpoint.transformKind == WVPersistedTransformKind::boussinesq)) {
