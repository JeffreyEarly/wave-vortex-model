...
WVKernelStatus WVHydrostaticIntegrationSystem::createImpl(
  ...
  if (descriptor != nullptr && descriptor->catalog() != catalog)
    return {WVKernelStatusCode::invalidConfiguration,
            ...
            "extension catalog."};
  try {
    if (!source) return invalid("Hydrostatic integration requires a scientific source.");
    const auto& configuration=source->geometry();
    auto candidate =
        std::unique_ptr<WVHydrostaticIntegrationSystem>(
    ...
    integrationBinder.advectedScalar =
      ...
      if (dimensions != std::vector<std::size_t>(
        ...
        return {WVKernelStatusCode::invalidShape,
                "A Hydrostatic tracer must have shape [Nx,Ny,Nz]."};
      ++ownerCounts[block];
      WVTracer tracer;
    ...
    integrationBinder.advectedPositions =
      ...
      if (observer.isXYOnly && observer.z.size() != observer.x.size())
        return invalid("Hydrostatic XY particles require one fixed "
    ...
    return {WVKernelStatusCode::allocationFailure,
            "Unable to allocate the Hydrostatic particle system."};
  }
}

WVHydrostaticIntegrationSystem::~WVHydrostaticIntegrationSystem() =
    default;

WVKernelStatus
WVHydrostaticIntegrationSystem::evaluateRightHandSide(
  ...
  if (executing_)
    return {WVKernelStatusCode::reentrantExecution,
            "The Hydrostatic integration system is not reentrant."};
...
WVHydrostaticIntegrationSystem::enforceStateConstraints(
...
WVKernelStatus
WVHydrostaticIntegrationSystem::initializeParticleState(
...
WVKernelStatus WVHydrostaticIntegrationSystem::createErrorPolicy(
...
WVKernelStatus
WVHydrostaticIntegrationSystem::evaluateFixedTimeStepCandidates(
...
std::size_t
WVHydrostaticIntegrationSystem::persistentBytes() const noexcept {
