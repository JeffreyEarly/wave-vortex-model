...
class StratifiedQGErrorPolicy final : public WVIntegrationErrorPolicy {
public:
  static WVKernelStatus create(
    ...
    try {
      auto policy = std::unique_ptr<StratifiedQGErrorPolicy>(
          new StratifiedQGErrorPolicy());
      const auto& geometry = kernel.geometry();
      struct Mode { double Kh; };
      std::vector<Mode> horizontal;
      for (std::size_t m = 0; m < geometry.Nkl; ++m) horizontal.push_back({std::hypot(geometry.k[m],geometry.l[m])});
      policy->tolerances_.assign(geometry.Nj*horizontal.size(), 1.0);
      ...
      uniqueKh.erase(std::unique(uniqueKh.begin(), uniqueKh.end()),
                     uniqueKh.end());
      if (uniqueKh.size() < 2)
        return invalid("Adaptive Stratified QG tolerances require at least "
      ...
      if (!(deltaK > 0.0) || !std::isfinite(deltaK))
        return invalid("Adaptive Stratified QG radial spacing is invalid.");
      ...
      for (std::size_t mode = 0; mode < horizontal.size(); ++mode) {
        ...
        if (bins[mode] == centers.size())
          return invalid("A Stratified QG mode was not assigned to an "
                         "adaptive radial bin.");
      }
      const auto &energy = kernel.factors().energy;
      for (std::size_t index = 0; index < energy.size(); ++index) {
        const auto mode = index/geometry.Nj;
        if (!(energy[index] > 0.0) || !std::isfinite(energy[index]))
          continue;
        ...
        policy->tolerances_[index] =
            absoluteToleranceScale *
            std::sqrt(energyPerCoefficient / energy[index]);
      }
      result = std::move(policy);
      return WVKernelStatus::ok();
    } catch (const std::bad_alloc &) {
      return {WVKernelStatusCode::allocationFailure,
              "Unable to allocate Stratified QG adaptive tolerances."};
    }
  }
...
WVKernelStatus WVStratifiedQGIntegrationSystem::createImpl(
  ...
  if (descriptor != nullptr && descriptor->catalog() != catalog)
    return invalid("The Stratified QG observer descriptor and numerical "
                   "system require the same extension catalog.");
  try {
    if (!source) return invalid("A stratified modal source is required.");
    const auto& configuration = source->geometry();
    auto candidate = std::unique_ptr<WVStratifiedQGIntegrationSystem>(
    ...
    WVTransformStateDescription stateDescription{
        "WVTransformStratifiedQG", {configuration.Nx, configuration.Ny, configuration.Nz},
        {{"A0", {configuration.Nj, configuration.Nkl},
    ...
    if (descriptor != nullptr) {
      ...
      binder.advectedPositions =
        ...
        if (!observer.isXYOnly)
          return {WVKernelStatusCode::unsupportedOperation,
                  "WVTransformStratifiedQG supports XY particles only."};
        if (observer.z.size() != observer.x.size())
          return {WVKernelStatusCode::unsupportedOperation,
                  "Stratified QG XY particles require a fixed depth per particle."};
        if (observer.stateBlockIdentifiers.size() != 2)
          return invalid("Stratified QG particles require x and y state "
                         "blocks only.");
        Particle particle;
        ...
        if (particle.xBlock == std::numeric_limits<std::size_t>::max() ||
          ...
          return invalid("Stratified QG particle state blocks are absent from "
      ...
      binder.advectedScalar =
          [&](const WVObserverRecord &observer) -> WVKernelStatus {
        if (!observer.isXYOnly)
          return {WVKernelStatusCode::unsupportedOperation,"MATLAB Stratified QG tracers require XY-only advection on the three-dimensional grid."};
        if (observer.stateBlockIdentifiers.size() != 1)
          return invalid("Stratified QG tracer state is not uniquely "
                         "identified.");
        ...
        if (block == std::numeric_limits<std::size_t>::max())
          return invalid("Stratified QG tracer state is absent from the "
                         "integration layout.");
        if (candidate->layout_.additionalBlocks()[block].dimensions !=
            std::vector<std::size_t>{configuration.Nx, configuration.Ny, configuration.Nz})
          return {WVKernelStatusCode::invalidShape,
                  "A Stratified QG tracer must have shape [Nx,Ny,Nz]."};
        ++ownerCounts[block];
      ...
      for (const auto &observer : descriptor->observers()) {
        ...
        if (resolved == nullptr)
          return {WVKernelStatusCode::unsupportedOperation,
                  "Stratified QG integration received an unresolved observer."};
      ...
      for (std::size_t block = 0; block < ownerCounts.size(); ++block)
        if (ownerCounts[block] != 1)
          return invalid("Integrated Stratified QG state block " +
    ...
    candidate->observerMetrics_.positionCapacityBytes =
        (candidate->x_.capacity() + candidate->y_.capacity() + candidate->z_.capacity()) *
        sizeof(double);
    ...
    return {WVKernelStatusCode::allocationFailure,
            "Unable to allocate the Stratified QG integration system."};
  }
}

WVKernelStatus WVStratifiedQGIntegrationSystem::evaluateRightHandSide(
    const WVIntegrationState &state,
    WVIntegrationFlux &rightHandSide) {
  if (executing_)
    return {WVKernelStatusCode::reentrantExecution,
            "The Stratified QG integration system is not reentrant."};
  ...
  if (rightHandSide.coefficientFamilyCount != 1 ||
    ...
    return {WVKernelStatusCode::invalidShape,
            "Stratified QG RHS storage does not match compact F0 and its "
  ...
  const auto spatial = kernel().spatialShape();
  for (const auto &tracer : tracers_) {
    ...
    const WVRealVolumeConstView scalar{input.realData, spatial};
    WVRealVolumeView scalarFlux{output.realData, spatial};
...
WVStratifiedQGIntegrationSystem::enforceStateConstraints(
  ...
  WVComplexView A0View{A0.data, kernel().spectralShape()};
  // MATLAB preserves the supplied A0 values, including horizontal means.
  return forcingEngine_->restoreForcingAmplitudes(A0View);
}

WVKernelStatus WVStratifiedQGIntegrationSystem::createErrorPolicy(
    double absoluteToleranceScale,
    std::unique_ptr<WVIntegrationErrorPolicy> &policy) const {
  std::unique_ptr<WVIntegrationErrorPolicy> coefficients;
  auto status = StratifiedQGErrorPolicy::create(
      kernel(), absoluteToleranceScale, coefficients);
  if (!status)
    return status;
  ...
  try {
    ...
    return {WVKernelStatusCode::allocationFailure,
            "Unable to allocate the Stratified QG observer error policy."};
  }
}

WVKernelStatus WVStratifiedQGIntegrationSystem::initializeParticleState(
...
WVKernelStatus
WVStratifiedQGIntegrationSystem::evaluateFixedTimeStepCandidates(
  ...
  if (!std::isfinite(result.maximumHorizontalSpeed) ||
    ...
    return invalid("Stratified QG CFL velocity is nonfinite or negative.");
  double maximumHorizontalWavenumber = 0.0;
  for (std::size_t m = 0; m < kernel().geometry().Nkl; ++m)
    maximumHorizontalWavenumber = std::max({maximumHorizontalWavenumber, std::abs(kernel().geometry().k[m]), std::abs(kernel().geometry().l[m])});
...
std::size_t
WVStratifiedQGIntegrationSystem::persistentBytes() const noexcept {
