...
int main(int argc, char** argv) {
    ...
    const auto stageStateConstructionSecondsBeforeSample = integrator.metrics().stageStateConstructionSeconds;
    const auto stageStateConstructionReadsBeforeSample = integrator.metrics().stageStateConstructionElementReads;
    const auto stageStateConstructionWritesBeforeSample = integrator.metrics().stageStateConstructionElementWrites;
    ...
    const auto sampledStageStateConstructionSeconds = integratorMetrics.stageStateConstructionSeconds-stageStateConstructionSecondsBeforeSample;
    const auto sampledStageStateConstructionReads = integratorMetrics.stageStateConstructionElementReads-stageStateConstructionReadsBeforeSample;
    const auto sampledStageStateConstructionWrites = integratorMetrics.stageStateConstructionElementWrites-stageStateConstructionWritesBeforeSample;
#if WV_RUNTIME_HAS_DENSE_OUTPUT
    ...
    report << std::setprecision(17)
           ...
           << "\"authorBenchmark\":{\"warmupStepCount\":" << options.benchmarkWarmupSteps << ",\"sampleStepCount\":" << (options.hasSteps ? options.steps : 0) << ",\"denseOutputsPerStep\":" << options.benchmarkDenseOutputsPerStep << ",\"interpolatedOutputCount\":" << interpolatedOutputCount << ",\"sampledStageStateConstructionReads\":" << sampledStageStateConstructionReads << ",\"sampledStageStateConstructionWrites\":" << sampledStageStateConstructionWrites << "},"
           << "\"forcing\":" << forcingJSON(checkpoint.forcingSchedule) << ','
           << "\"timingSeconds\":{\"inspect\":" << timings.inspect << ",\"read\":" << timings.read << ",\"construct\":" << timings.construct << ",\"prepare\":" << timings.prepare << ",\"integrate\":" << timings.integrate << ",\"stageStateConstruction\":" << sampledStageStateConstructionSeconds << ",\"write\":" << timings.write << ",\"total\":" << timings.total << "},"
