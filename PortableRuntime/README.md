...
The private `WVDensityEventEvaluation` helper joins recovery and profile calculus inside the existing field evaluator. Its qualification factory accepts full-natural-grid `rho_nm`, `eta_true` and `ape` requests; public field-plan resolution still rejects those names. One output event owns reconstructed total density and the geometry it borrows into the helper. Sequential consumers share completed recovery, profile construction and material-height inversion, even if their individual plans are destroyed between calls. Direct evaluation creates an invocation-local scope; subsequent invocations always rebuild derived density state, including when coefficient addresses and times are unchanged. Ordinary density plans request the same event lifetime without requiring forcing or moving observers. Complete writer-routing and restart qualification remain separate gates before public output promotion.

`rho_nm` always denotes the actual recovered profile. The explicit initial reference selects the profile used for displacement and APE only: mixed requests can recover actual `rho_nm` while evaluating displacement/APE against the initial profile. An initial-reference request without `rho_nm` does not recover a profile; a profile-only request does not invert parcels. Reference selection is immutable within a resolved plan and preserved by rebinding. Mixing reference selections within one active event is rejected. All active output views are validated before work, and their values are published only after every requested numerical stage succeeds. Event counters expose recovery, profile construction, inverse and APE passes and reuse; live storage returns to zero when the event or direct invocation ends. Density-only evaluation avoids retaining the unrelated four-volume primitive cache.
