...
`WVConstantStratificationCompositeSystem` integrates MATLAB-compatible `WVLagrangianParticles` beside the canonical coefficients. Particle records resolve ordered x/y/optional-z blocks at construction. Every RHS evaluation asks the forcing engine for one generation-checked context containing the `u`, `v`, and `w` fields already reconstructed by `nonlinearFlux`; every particle system samples that same immutable view. If a forcing schedule does not reconstruct velocity, the context prepares it once on demand. XY-only particles retain configured z values without integrating them. Accepted horizontal coordinates are never wrapped or clamped; periodic reduction belongs solely to interpolation. Fixed RK4, adaptive RK3(2), rejected-step semantics, and dense output therefore operate on particle blocks without particle-specific logic in the numerical methods.
