...
`WVObserverOutputEvaluationService` adds the passive observing-system contract above field evaluation. It exposes `WVCoefficients`, `WVEulerianFields`, and `WVMooring` using their MATLAB class names, variable names, logical dimensions, units, descriptions, and linear/nonlinear cadence. Coefficients are borrowed directly from the immutable event state. Coincident observers share each unique field/sampling request, and only the requested result buffers are retained. Mooring positions are wrapped periodically and converted once to MATLAB's one-based nearest-grid indices; output uses `[z,id,t]` logical order with static `<name>_id`, `<name>_z`, `<name>_x`, and `<name>_y` coordinates.
