...
`WVModelOutputNetCDFSink` implements that sink contract with the existing MATLAB 4.x output-file semantics. It stages every new destination before committing the file set, stores one unlimited `t` dimension per named output group, writes real/imaginary complex pairs in `[t,kl,j]` NetCDF order, and persists only observer configuration plus required particle/tracer state. Payload variables are written before `t`; after `t` is committed the file is synchronized. Inspection and append reject fill-valued partial records, off-lattice or duplicate times, incompatible schedules and shapes, unsupported observer tags, broken shared identities, and files without exactly one complete coefficient restart group. `inspect()` reconstructs the portable descriptor, schedule progress, latest canonical state, and required dynamic observer state from MATLAB- or runtime-produced files. Plans, caches, dense-output history, controller state, and scratch are never persisted.
