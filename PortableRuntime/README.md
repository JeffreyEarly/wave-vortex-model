...
This directory contains the optional MATLAB-independent portable runtime. MATLAB remains WaveVortexModel's primary interface. The qualified `WVModel`, field, observer, NetCDF, and restart composition supports constant stratification, `WVTransformBarotropicQG` and `WVTransformStratifiedQG` behind transform-neutral state and service contracts. Barotropic QG retains MATLAB's compact `A0`-only `kl` layout through integration, output, append, and restart; it never allocates compatibility `Ap` or `Am` state. The runtime is a source-only checkpoint-to-checkpoint tool for advanced users and the v1 source-level surface for statically linked C++ observing-system, output-schedule, and forcing implementations.
...
The shared stratified field service provides standard fields, diagnostics, surface variables and fixed, moving and event-based linear/spline interpolation. Integrated observers include 3D tracers with full or horizontal-only advection, and particles with full or fixed-depth XY advection; moorings, Eulerian fields and tracked variables use the existing output providers. Explicit/CFL RK4 and adaptive RK3(2)/RK7(8) use the existing integration and output machinery, including dense output, multi-file graphs, create/replace/append, restart and transactional preflight.
...
`contracts/portable-forcing-compatibility-v1.json` is the forcing slice of `portable-compatibility-matrix-v1`. Its 120 rows cover the twelve stable supplied MATLAB forcings, constant stratification in hydrostatic/nonhydrostatic mode, Barotropic QG, Stratified QG and Hydrostatic, with transform antialiasing on/off. There are 101 supported pairs and 19 intentional MATLAB incompatibilities. Thermal damping remains explicitly excluded as under development. Later transform integration issues extend this same slice; #306 assembles the full feature catalog.
...
Pass this scientific owner to `WVTransformBoussinesqKernel` for the [standalone numerical surface](../CompiledKernel/README.md#boussinesq-numerical-kernel). Issue #302 adds kernel capability; complete-model Boussinesq dispatch remains unsupported until #303, with end-to-end qualification in #304. MATLAB production APIs, scientific methods and save defaults are unchanged.
