...
`contracts/portable-forcing-compatibility-v1.json` is the forcing slice of `portable-compatibility-matrix-v1`. Its 96 rows cover the twelve stable supplied MATLAB forcings, constant stratification in hydrostatic/nonhydrostatic mode, Barotropic QG and Stratified QG, with transform antialiasing on/off. There are 78 supported pairs and 18 intentional MATLAB incompatibilities. Thermal damping remains explicitly excluded as under development. Later transform integration issues extend this same slice; #306 assembles the full feature catalog.
