...
The shared stratified field service provides standard fields, diagnostics, surface variables and fixed, moving and event-based linear/spline interpolation. Integrated observers include 3D tracers and particles, including fixed-depth XY particles; moorings, Eulerian fields and tracked variables use the existing output providers. Explicit/CFL RK4 and adaptive RK3(2)/RK7(8) use the existing integration and output machinery, including dense output, multi-file graphs, create/replace/append, restart and transactional preflight.
