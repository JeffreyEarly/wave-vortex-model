...
- Added method-neutral composite dense-output/integrator boundaries and guarded, descriptor-compatible multi-file, named-group output orchestration with original-lattice ordinals, exact coincident-time aggregation, resumable failed-route staging, immutable route views, structured attempt/failure records, and exact delivery metrics.
...
- Qualified all five built-in observing systems for bidirectional MATLAB-compatible multi-file output, restart, continuation, and append with fixed RK4 or adaptive RK3(2); custom observing-system subclasses remain unsupported.
