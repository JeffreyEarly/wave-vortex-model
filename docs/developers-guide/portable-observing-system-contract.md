...
Output-file records identify a destination. Ordered output-group records identify a name, interval and time bounds, observer references, and which single group carries a complete coefficient restart. File-specific NetCDF dimensions, variables, and attributes are deliberately deferred to the observer evaluators and MATLAB-compatible writer issues; this contract supplies the stable information from which they must be derived.
