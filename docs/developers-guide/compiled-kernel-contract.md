...
The `1474.59` ratio recorded during same-host validation on a small `32` by `24` grid compares native FFTW with the deliberately slow direct-DFT correctness provider. It is provider/oracle validation, not a MATLAB comparison, an end-to-end runtime measurement, or a product-performance claim. The direct-DFT provider exists to supply a transparent numerical oracle and a portable development fallback at small sizes.
