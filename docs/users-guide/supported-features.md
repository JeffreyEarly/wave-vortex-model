...
The exported package includes the MATLAB-independent C++ core and portable runtime as source. On Apple silicon, users may build the standalone runner locally with `PortableRuntime/buildWaveVortexRun.sh`; no FFTW archive, native library, MEX file, or executable is distributed.
