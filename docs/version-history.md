...
- Added method-neutral composite dense-output/integrator boundaries and preflighted multi-file, named-group output orchestration with original-lattice ordinals, continuation progress, coincident-time aggregation, immutable route views, bounded staging, structured partial-failure records, and exact delivery metrics; NetCDF multi-group persistence remains separate.
