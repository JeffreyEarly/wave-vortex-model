classdef TestPortableBoussinesqQualificationEvidence < matlab.unittest.TestCase
    ...
    methods (TestClassSetup)
        function loadRecordedEvidence(testCase)
            ...
            testCase.report = jsondecode(fileread(fullfile(root,"PortableRuntime","qualification","boussinesq-apple-silicon-v1.json")));
        end
    end
    methods (Test,TestTags="full")
        ...
        function contractsCannotClaimCompleteContinuationQualification(testCase)
            ...
            grids = reshape([referenceReport.lifecycle.grid],3,[])';
            referenceReport.lifecycle = referenceReport.lifecycle(string({referenceReport.lifecycle.provider})=="reference" & ismember(grids,[8 6 9;12 10 13],"rows")');
            referenceReport.schemaIdentifier = "wave-vortex-boussinesq-contracts-v1";
        ...
        function missingOrContradictoryEvidenceIsRejected(testCase)
            ...
            bad = original; bad.rows(1).testNames="TestPortableBoussinesq/fieldSamplingMatchesMatlab"; testCase.verifyInvalid(bad);
        ...
        function numericalAndMemoryFailuresCannotClaimReadiness(testCase)
            ...
            bad = testCase.report; bad.continuations(1).matlabRestart.fields=1; testCase.verifyInvalid(bad);
            bad = testCase.report; bad.continuations(1).verticalKineticFraction=NaN; testCase.verifyInvalid(bad);
            bad = testCase.report; bad.continuations(1).runtime(1).qualificationScientificStatePreserved=false; testCase.verifyInvalid(bad);
            bad = testCase.report; bad.continuations(1).runtime(1).state.finalTime=1; testCase.verifyInvalid(bad);
            bad = testCase.report; bad.continuations(1).runtime(1).state.rhsEvaluationCount=0; testCase.verifyInvalid(bad);
