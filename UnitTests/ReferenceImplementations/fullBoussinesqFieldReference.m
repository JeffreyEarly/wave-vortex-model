function fields = fullBoussinesqFieldReference(self,variableNames,options)
...
spectral = fullBoussinesqSpectralReference(self,flowComponent=options.flowComponent);
sampled = struct();
...
if any(ismember(variableNames,["u","v","w","ssu","ssv","w_i","z_physical"]))
    ...
    if ~isempty(options.flowComponent)
        total = fullBoussinesqSpectralReference(self);
