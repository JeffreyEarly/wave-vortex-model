classdef TestPortableForcingCompatibility < matlab.unittest.TestCase
    ...
    methods (Test,TestTags="full")
        ...
        function incompleteEvidenceCannotClaimReadiness(testCase)
            validatePortableForcingCompatibility(testCase.matrix);
            testCase.verifyError(@()validatePortableForcingCompatibility(testCase.matrix,requireComplete=true),"WaveVortexModel:CompatibilityIncomplete");
            bad = testCase.matrix;
            bad.rows(1).acceptance = 'supported';
            testCase.verifyInvalid(bad);
            bad = testCase.matrix;
            bad.completion = 'complete';
            testCase.verifyInvalid(bad);
        end
        ...
        function missingImplementationsAndScientificRejectionsStayDistinct(testCase)
            ...
            for family = ["constant-hydrostatic","constant-nonhydrostatic","barotropic"]
                ...
                testCase.verifyFalse(antialias.implementation.factoryAvailable);
                testCase.verifyEqual(antialias.implementation.issue,290);
            ...
            testCase.verifyEqual(wave.implementation.issue,293);
