classdef TestPortableHydrostatic < matlab.unittest.TestCase
    ...
    methods (Test,TestTags="full")
        ...
        function incompatibleModelGraphsAreTransactional(testCase)
            ...
            rows = catalog.rows(startsWith(string({catalog.rows.configuration}),"hydrostatic") & string({catalog.rows.acceptance}) == "incompatible");
        ...
        function fieldSamplingMatchesMatlab(testCase)
            ...
            wvt = WVTransformHydrostatic([17000 11000 1000],[9 7 9],Nj=4,z=z,N2Function=@(z) 1e-4*exp(z/700),shouldAntialias=false);
        ...
        function retainedSubsetAndNonuniformGrid(testCase)
            ...
            base = WVTransformHydrostatic([17000 11000 1000],[9 7 12],Nj=8,z=z,N2Function=@(z) 5e-5*(1+.4*tanh((z+400)/180)),shouldAntialias=false,latitude=-33);
            keep = [1 3 5 7];
        ...
        function malformedModelsLeaveAppendTargetsUnchanged(testCase)
            for identity = ["WVAntialiasing","WVHorizontalDamping","WVVerticalDamping","WVVerticalDiffusivity","modal source"]
                wvt = testCase.transform([8 6 9],false);
                if identity=="modal source", wvt.setForcing(WVNonlinearAdvection(wvt));
                else, wvt.setForcing(testCase.forcing(wvt,identity)); end
                source = testCase.writeInitialModel(wvt,true);
                if identity=="modal source"
        ...
        function applicableForcingTendenciesMatchMatlab(testCase)
            ...
            for grid = {[8 6 9],[9 7 10]}
                for antialias = [false true]
                    ...
                    rows = catalog.rows(string({catalog.rows.configuration}) == "hydrostatic-aa"+double(antialias) & string({catalog.rows.acceptance}) == "supported");
                    for row = reshape(rows,1,[])
    ...
    methods (Access=private)
        function wvt = transform(testCase,grid,antialias,options)
            ...
            wvt = WVTransformHydrostatic([17000 11000 1000],grid,Nj=options.Nj,N2Function=@(z) 1e-4*exp(z/700),shouldAntialias=antialias);
            testCase.seedState(wvt);
        end
