classdef TestPortableHydrostaticQualification < matlab.unittest.TestCase
    ...
    methods (TestClassSetup)
        function prepareRuntime(testCase)
            ...
            testCase.evidenceFolder = string(getenv("WV_HYDRO_EVIDENCE_FOLDER"));
            ...
            if testCase.runner == ""
                ...
                [status,output] = cleanSystem("cmake --build "+shellQuote(build)+" --parallel 4 --target wave-vortex-run WVHydrostaticLifecycleProbe");
                testCase.assertEqual(status,0,output);
    ...
    methods (Test,TestTags="full")
        function longerContinuationMatchesMatlab(testCase)
            root = string(fileparts(fileparts(mfilename("fullpath"))));
            manifest = jsondecode(fileread(fullfile(root,"PortableRuntime","contracts","hydrostatic-qualification-cases-v1.json")));
            for definition = reshape(manifest.cases,1,[])
                definition.grid = reshape(definition.grid,1,[]);
                [source,initialState] = testCase.authorModel(definition);
                for provider = testCase.providers
                    ...
                    clear cleanup
                    row = struct(definition=definition,provider=provider,whole=whole,segmented=split,coefficientChange=change,runtime={reports});
        ...
        function lifecycleAndStorageRemainBounded(testCase)
            executable = fullfile(fileparts(testCase.runner),"WVHydrostaticLifecycleProbe");
    ...
    methods (Access=private)
        function [path,initialState] = authorModel(testCase,definition)
            ...
            wvt = WVTransformHydrostatic([17000 11000 1000],definition.grid,Nj=definition.Nj,z=z,N2Function=N2Function,shouldAntialias=definition.shouldAntialias);
            ...
            model.addParticles('drifter',definition.xyOnly,[16990 5000],[10995 1000],[-600 -100],'u','qgpv',advectionInterpolation="spline",trackedVarInterpolation="linear");
