classdef TestPerKappaWavePersistence < matlab.unittest.TestCase
    methods (Test, TestTags="full")
        ...
        function legacyUniformFileDefaultsToFullPrefixes(testCase)
            fixture=testCase.applyFixture(matlab.unittest.fixtures.TemporaryFolderFixture);
            w=WVTransformFreeSurfaceBoussinesq.fromStratification([1e5 1e5 1000],[8 8 65],shouldAntialias=true,N2Function=@(z)1e-4+0*z,apvModeCount=3,mdaModeCount=2,waveModeCount=4,inertialModeCount=3);
            populate(w);
...
base=WVTransformFreeSurfaceBoussinesq.fromStratification(Lxyz,Nxyz,shouldAntialias=true,N2Function=N2,apvModeCount=3,mdaModeCount=2,waveModeCount=4,inertialModeCount=3);
counts=mod((1:length(base.khUnique)).',5);
if zeroWaves, counts(:)=0; end
w=WVTransformFreeSurfaceBoussinesq.fromStratification(Lxyz,Nxyz,shouldAntialias=true,N2Function=N2,waveModeKappa=base.khUnique,waveModeCount=counts,inertialModeCount=2,apvModeCount=3,mdaModeCount=2);
end
