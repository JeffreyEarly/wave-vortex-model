...
for iOp = 1:length(variableName)
    ...
    switch name
        case 'rho_nm'
            operations(iOp) = WVNoMotionProfileOperation();
        ...
        otherwise
            switch name
                ...
                case 'A0t'
                    varAnnotation = WVVariableAnnotation(name,options.spectralDimensionNames,'m2 s-1', 'zero-frequency coefficients at current time t');
                    varAnnotation.isComplex = true;
                    f = @(wvt) wvt.A0;

                case 'Apt'
                    varAnnotation = WVVariableAnnotation(name,options.spectralDimensionNames,'m s-1', 'positive-frequency coefficients at current time t');
                    varAnnotation.isComplex = true;
                    f = @(wvt) wvt.Ap .* wvt.phase;

                case 'Amt'
                    varAnnotation = WVVariableAnnotation(name,options.spectralDimensionNames,'m s-1', 'negative-frequency coefficients at current time t');
                    varAnnotation.isComplex = true;
                    f = @(wvt) wvt.Am .* wvt.conjPhase;

                case 'u'
                    varAnnotation = WVVariableAnnotation(name,options.spatialDimensionNames,'m s-1', 'x-component of the fluid velocity');
                ...
                case 'v'
                    varAnnotation = WVVariableAnnotation(name,options.spatialDimensionNames,'m s-1', 'y-component of the fluid velocity');
                ...
                case 'w'
                    varAnnotation = WVVariableAnnotation(name,options.spatialDimensionNames,'m s-1', 'z-component of the fluid velocity');
                ...
                case 'p'
                    varAnnotation =  WVVariableAnnotation(name,options.spatialDimensionNames,'kg m-1 s-2', 'pressure anomaly');
                    f = @(wvt) wvt.rho0*wvt.g*transformToSpatialDomainWithF(wvt,@(wvt) wvt.NAp.*wvt.Apt,@(wvt) wvt.NAm.*wvt.Amt,@(wvt) wvt.PA0.*wvt.A0t);

                case 'psi'
                    varAnnotation =  WVVariableAnnotation(name,options.spatialDimensionNames,'m2 s-1', 'geostrophic streamfunction');
                ...
                case 'qgpv'
                    varAnnotation =  WVVariableAnnotation(name,options.spatialDimensionNames,'s-1', 'quasigeostrophic potential vorticity');
                    varAnnotation.isVariableWithLinearTimeStep = false;
                    varAnnotation.isVariableWithNonlinearTimeStep = true;
                ...
                case 'rho_e'
                    varAnnotation = WVVariableAnnotation('rho_e',options.spatialDimensionNames,'kg m-3', 'excess density');
                ...
                case 'rho_total'
                    varAnnotation = WVVariableAnnotation('rho_total',options.spatialDimensionNames,'kg m-3', 'total potential density');
                    f = @(wvt) reshape(wvt.rho_nm0,1,1,[]) + wvt.rho_e;

                case 'zeta_x'
                    varAnnotation = WVVariableAnnotation('zeta_x',options.spatialDimensionNames,'s-1', 'x-component component of relative vorticity');
                    f = @(wvt) wvt.diffY(wvt.w) - wvt.diffZF(wvt.v);

                case 'zeta_y'
                    varAnnotation = WVVariableAnnotation('zeta_y',options.spatialDimensionNames,'s-1', 'y-component component of relative vorticity');
                    f = @(wvt) wvt.diffZF(wvt.u) - wvt.diffX(wvt.w);

                case 'zeta_z'
                    varAnnotation = WVVariableAnnotation('zeta_z',options.spatialDimensionNames,'s-1', 'vertical component of relative vorticity');
                    varAnnotation.attributes('short_name') = 'ocean_relative_vorticity';
                    f = @(wvt) wvt.diffX(wvt.v) - wvt.diffY(wvt.u);

                case 'ssu'
                    varAnnotation = WVVariableAnnotation('ssu',{'x','y'},'m s-1', 'x-component of the fluid velocity at the surface',detailedDescription='- topic: State Variables');
                    f = @(wvt) surfaceSlice(transformToSpatialDomainWithF(wvt,@(wvt) wvt.UAp.*wvt.Apt,@(wvt) wvt.UAm.*wvt.Amt,@(wvt) wvt.UA0.*wvt.A0t));

                case 'ssv'
                    varAnnotation = WVVariableAnnotation('ssv',{'x','y'},'m s-1', 'y-component of the fluid velocity at the surface',detailedDescription='- topic: State Variables');
