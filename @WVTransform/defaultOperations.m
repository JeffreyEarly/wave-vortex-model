...
outputVar(1) = WVVariableAnnotation('Apt',{'j','kl'},'m s-1', 'positive wave coefficients at time t');
outputVar(1).isComplex = 1;

outputVar(2) = WVVariableAnnotation('Amt',{'j','kl'},'m s-1', 'negative wave coefficients at time t');
outputVar(2).isComplex = 1;

f = @(wvt) wvt.waveCoefficientsAtTimeT();
operations(end+1) = WVOperation('ApAm',outputVar,f);

outputVar = WVVariableAnnotation('A0t',{'j','kl'},'m2 s-1', 'zero-frequency coefficients at current time t');
...
% outputVar = WVVariableAnnotation('u',{'x','y','z'},'m s-1', 'x-component of the fluid velocity');
...
% 
% outputVar = WVVariableAnnotation('v',{'x','y','z'},'m s-1', 'y-component of the fluid velocity');
...
% outputVar = WVVariableAnnotation('w',{'x','y','z'},'m s-1', 'z-component of the fluid velocity');
...
% outputVar = WVVariableAnnotation('p',{'x','y','z'},'kg m-1 s-2', 'pressure anomaly');
...
% outputVar = WVVariableAnnotation('psi',{'x','y','z'},'m2 s-1', 'geostrophic streamfunction');
...
outputVar = WVVariableAnnotation('rho_total',{'x','y','z'},'kg m-3', 'total potential density');
f = @(wvt) reshape(wvt.rho_nm0,1,1,[]) + wvt.rho_e;
operations(end+1) = WVOperation('rho_total',outputVar,f);

% outputVar = WVVariableAnnotation('qgpv',{'x','y','z'},'s-1', 'quasigeostrophic potential vorticity');
% f = @(wvt) wvt.transformToSpatialDomainWithF( A0=wvt.A0_QGPV_factor .*wvt.A0t);
% operations(end+1) = WVOperation('qgpv',outputVar,f);
% 
% outputVar = WVVariableAnnotation('Lr2',{'j'},'m2', 'squared Rossby radius');
...
outputVar = WVVariableAnnotation('ssu',{'x','y'},'m s-1', 'x-component of the fluid velocity at the surface',detailedDescription='- topic: State Variables');
operations(end+1) = WVOperation('ssu', outputVar,@(wvt) wvt.u(:,:,end));

outputVar = WVVariableAnnotation('ssv',{'x','y'},'m s-1', 'y-component of the fluid velocity at the surface',detailedDescription='- topic: State Variables');
...
fluxVar(1) = WVVariableAnnotation('Fp',{'j','kl'},'m s-2', 'nonlinear tendency of Ap', isComplex=1, detailedDescription='- topic: State Variables');
fluxVar(2) = WVVariableAnnotation('Fm',{'j','kl'},'m s-2', 'nonlinear tendency of Am', isComplex=1,detailedDescription='- topic: State Variables');
fluxVar(3) = WVVariableAnnotation('F0',{'j','kl'},'m2 s-2', 'nonlinear tendency of A0', isComplex=1,detailedDescription='- topic: State Variables');
