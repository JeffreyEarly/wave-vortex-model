...
int main(int argc, char **argv) {
  ...
  const double readSeconds = std::chrono::duration<double>(Clock::now()-processStarted).count();
  ...
  const double constructionSeconds = std::chrono::duration<double>(Clock::now()-constructionStarted).count();
  ...
  // The retained payload is the last authoritative uninstrumented result.
  // Stage instrumentation is a separate call after the authoritative samples.
  forcing->kernel().setStageInstrumentation(true);
  status = forcing->nonlinearFlux(state, flux);
  if (!status) { emitFailure("instrumented-execute", status.message); return 5; }
  ...
  std::cout << "]},\"memory\":{\"baselineProcessBytes\":" << baselineRSS
            ...
            << ",\"checksum\":" << checksum << "},\"adoption\":{\"schedule\":\""
            << escape(forcing->kernel().nonlinearFluxScheduleIdentifier())
            << "\",\"readSeconds\":" << readSeconds
            << ",\"constructionSeconds\":" << constructionSeconds
            << ",\"kernelPersistentBytes\":" << forcing->kernel().persistentBytes()
            << ",\"scratchCapacityBytes\":" << metrics.scratchCapacityBytes
            << ",\"halfSpectrumCapacityBytes\":" << metrics.halfSpectrumScratchCapacityBytes
            << ",\"realCapacityBytes\":" << metrics.realScratchCapacityBytes
            << ",\"planBytesLowerBound\":" << metrics.planBytes
            << ",\"engineBytesLowerBound\":" << metrics.engineBytes
            << ",\"coefficientWorkers\":" << forcing->kernel().coefficientWorkerCount()
#if defined(WV_KERNEL_COMPACT_CONSTANT_CANDIDATE)
            << ",\"pointwiseWorkers\":" << forcing->kernel().pointwiseWorkerCount()
            << ",\"retainedHorizontalWorkers\":" << forcing->kernel().retainedHorizontalWorkerCount()
#else
            << ",\"pointwiseWorkers\":1,\"retainedHorizontalWorkers\":0"
#endif
            << ",\"reconstructionSeconds\":" << metrics.reconstructionSeconds
            << ",\"derivativeReconstructionSeconds\":" << metrics.derivativeReconstructionSeconds
            << ",\"productSeconds\":" << metrics.productSeconds
            << ",\"projectionSeconds\":" << metrics.projectionSeconds
            << ",\"phaseSeconds\":" << metrics.phaseSeconds
            << ",\"coefficientAssemblySeconds\":" << metrics.coefficientAssemblySeconds
            << ",\"derivativeCoefficientAssemblySeconds\":" << metrics.derivativeCoefficientAssemblySeconds
            << ",\"coefficientProjectionSeconds\":" << metrics.coefficientProjectionSeconds
            << "}}\n";
