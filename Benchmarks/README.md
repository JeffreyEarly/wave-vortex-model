...
`runThreeInterfaceBenchmarkComparison` is the matched complete-workflow companion. Its canonical `[256 256 129]` and `[512 512 257]` nonhydrostatic cases compare MATLAB builtin, MATLAB with the compiled core, and standalone C++ execution for one `nonlinearFlux`, one fixed-RK4 continuation, and one adaptive RK3(2) continuation with observer output. Every sample uses a fresh process and an identical frozen model file. MATLAB builtin uses MATLAB transforms; both compiled interfaces use the same validated native FFTW provider. Public presentation uses only numerical-operation or integration runtime and total peak process-tree RSS; process-wall timing and secondary RSS diagnostics remain in the compact record. Publication requires provider identity, exact requested/actual integrator agreement, numerical tolerance, comparable memory evidence, and complete NetCDF output-graph agreement, including fields, moorings, particles, tracers, metadata, dimensions, schedules, and record counts.
...
The detailed raw result and RSS samples are compressed beneath the external sibling archive `../wave-vortex-model-benchmark-artifacts/three-interface/`. They are never committed or copied into the generated website. The source tree contains only the compact normalized record and presentation; its provenance stores the external archive filename, SHA-256, and compressed byte count. The author-only standalone kernel worker is built only when `WV_RUNTIME_BUILD_BENCHMARKS=ON`; it is not part of the package or ordinary `wave-vortex-run` interface.
