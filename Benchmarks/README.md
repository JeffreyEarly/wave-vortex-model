...
`runCompiledKernelAssemblyDecisionBenchmark` is the final issue #131 gate. It builds uniquely named modules from the original compiled baseline, native-FFTW foundation, preceding adopted core, and final assembled source, then compares each with production MATLAB `nonlinearFlux` in rotated fresh-process runs. Exact memory excludes the shared canonical state, includes the three returned flux arrays, and reports MATLAB FFT and FFTW plan-owned memory separately as opaque. The artifact records `CORE-COMPLETE` independently from `INTEGRATION-READY`, `MEMORY-ONLY`, or `INTEGRATION-NOT-READY`.
