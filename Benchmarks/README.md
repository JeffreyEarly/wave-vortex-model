...
`runThreeInterfaceBenchmarkComparison` is the matched complete-workflow companion. At `[256 256 129]` and `[512 512 257]`, it compares MATLAB builtin, MATLAB with the compiled core, and standalone C++ execution for fixed RK4 and MATLAB-compatible RK3(2), RK5(4), and RK8(7) integration. The matrix includes hydrostatic and nonhydrostatic constant stratification and pairs a coefficient-only endpoint workload with a composite graph containing fields, particles, a tracer, source-linked mooring state, and three scheduled interior dense-output points per maximum-size step. The canonical step is the exactly representable `1/1024`; its quarter-step output lattice avoids cumulative binary rounding drift at accepted-step boundaries. One frozen MATLAB-authored model file is shared by all interfaces for each physical workload. MATLAB builtin uses production MATLAB transforms; both compiled interfaces use the same validated native FFTW provider. Public presentation uses only integration runtime and total process-tree peak RSS sampled during the integration phase. Startup, construction, planning, parsing, and cleanup are excluded from the primary metrics.

Publication requires exact requested/actual integrator and provider identity, matched adaptive step bounds and tolerances, phase-scoped memory evidence, method-appropriate numerical tolerance, unchanged accepted endpoint trajectories when dense output is added, and complete NetCDF output-graph agreement. Compact diagnostics retain accepted/rejected steps, RHS and FSAL work, dense-extension evaluations, secondary RSS measurements, exact standalone persistent/workspace/history/lazy-extension byte ledgers, and every state-sized buffer's producer and last consumer. MATLAB solver storage, allocator/COW behavior, and FFT/provider storage remain explicitly opaque where exact attribution is unavailable.

Run one resolution directly with `runThreeInterfaceBenchmark`, or collect both canonical resolutions sequentially with:
