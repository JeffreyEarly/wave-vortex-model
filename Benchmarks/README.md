...
`runThreeInterfaceBenchmark` is the matched complete-workflow companion. Its canonical `[256 256 129]` nonhydrostatic case compares MATLAB builtin, MATLAB compiled, and standalone compiled execution for one `nonlinearFlux`, one fixed-RK4 continuation, and one adaptive-RK3(2) continuation with observer output. Every sample uses a fresh process and an identical frozen model file. The result reports total peak process-tree RSS from launch through completion as the primary practical interface footprint; retained-baseline increment and final RSS are secondary diagnostics. Publication requires exact requested/actual integrator agreement and complete NetCDF output-graph agreement, including fields, moorings, particles, tracers, metadata, dimensions, schedules, and record counts.
