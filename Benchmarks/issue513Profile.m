function report = issue513Profile(fixturePath,outputPath,options)
% Run the issue 513 matched-model diagnostic from an immutable fixture.
arguments
...
if isfile(outputPath)
    error("WaveVortexBenchmark:Issue513OutputExists","Refusing to overwrite the diagnostic output: %s",outputPath)
end
...
if double(model.t) ~= 0
    error("WaveVortexBenchmark:Issue513FixtureTime","The copied fixture must restart at t=0; found %.17g.",double(model.t))
end
...
model.odeOptions = odeset(model.odeOptions,'InitialStep',418.31529239621887,'Stats','on');
...
integrator = struct( ...
    ...
    "requestedInitialStep",418.31529239621887, ...
...
if options.shouldProfile
    profileLabel = "issue 513 matched adaptive ode78";
...
if integrator.reportedRightHandSideEvaluationCount ~= rhsEvaluationCount
    error("WaveVortexBenchmark:Issue513WorkCount","MATLAB ode78 reported %d function evaluations, but WVModel counted %d.",integrator.reportedRightHandSideEvaluationCount,rhsEvaluationCount)
end
...
report = struct( ...
    "schemaVersion","wvm-issue513-profile-v1", ...
    "fixture",fixture, ...
    "outputPath",outputPath, ...
    "backend",options.backend, ...
    "controls",struct("integrator","ode78","initialTime",0,"finalTime",7168,"initialStep",418.31529239621887,"relativeTolerance",1e-3,"absoluteTolerance",1e-6,"maximumStep",716.8,"maximumStepPolicy","MATLAB default: 0.1 integration span"), ...
...
if options.reportPath == "", options.reportPath = outputPath+".issue513.mat"; end
save(options.reportPath,"report","profileAnalysis","-v7.3");
if options.jsonReportPath == "", options.jsonReportPath = outputPath+".issue513.json"; end
...
if isempty(token), error("WaveVortexBenchmark:Issue513Statistics","MATLAB ode78 did not report %s.",label); end
...
if string(metadata.activeBackend) ~= backend
    error("WaveVortexBenchmark:Issue513BackendFallback","Requested %s but %s executed.",backend,string(metadata.activeBackend))
end
if backend == "compiled" && (string(metadata.provider.id) ~= "native-neon-pthreads" || ~metadata.module.identityValidated || metadata.libraries.openmp.detected)
    error("WaveVortexBenchmark:Issue513Provider","The compiled run did not use the validated native provider.")
end
end
...
endpointHash = "0dcc972706406cf25e441261f3079413b6e50650874515cbba2e60082af556ef";
compositeHash = "0d1d70dbff2a8ce29316f22b397a1a1c228bb50394c87da2ab9f463788c49e91";
...
if status ~= 0, error("WaveVortexBenchmark:Issue513HashFailed","%s",output); end
...
for dependencyPath = reshape(dependencyPaths,1,[])
    if ~isfolder(dependencyPath), error("WaveVortexBenchmark:Issue513Dependency","Dependency folder does not exist: %s",dependencyPath); end
...
if exist("profileCodeHotspots","file") ~= 2
    error("WaveVortexBenchmark:Issue513ProfilingTools","Add OceanKit/tools/profiling with profilingToolsPath when shouldProfile is true.")
end
end

function validateScientificFixture(wvt,pathname,workload)
valid = isa(wvt,"WVTransformConstantStratification") && isequal([wvt.Lx wvt.Ly wvt.Lz],[150e3 150e3 1300]);
valid = valid && isequal([wvt.Nx wvt.Ny wvt.Nz],[256 256 129]) && wvt.N0 == sqrt(2e-5) && wvt.latitude == 45;
valid = valid && ~wvt.isHydrostatic && wvt.shouldAntialias;
if ~valid
    error("WaveVortexBenchmark:Issue513ScientificFixture","The copied fixture does not have the frozen constant nonhydrostatic 256-by-256-by-129 scientific configuration.")
end
...
if workload == "coefficient-endpoint" && ~isequal(recordCounts,[1 0 0 0])
    error("WaveVortexBenchmark:Issue513FixtureOutputs","The endpoint fixture does not contain exactly its one initial coefficient record.")
elseif workload == "composite-dense-output" && ~isequal(recordCounts,[1 1 1 1])
    error("WaveVortexBenchmark:Issue513FixtureOutputs","The composite fixture does not contain exactly one initial record in each output group.")
elseif workload == "unknown"
    error("WaveVortexBenchmark:Issue513FixtureWorkload","Name a regenerated fixture with coefficient-endpoint or composite-dense-output so its output contract can be checked.")
end
end
...
if fileId < 0, error("WaveVortexBenchmark:Issue513Phase","Unable to write phase file: %s",pathname); end
...
fileId = fopen(pathname,"w");
if fileId < 0, error("WaveVortexBenchmark:Issue513Report","Unable to write report: %s",pathname); end
cleanup = onCleanup(@()fclose(fileId));
