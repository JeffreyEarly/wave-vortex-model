...
Optimize full-grid horizontal derivatives in the compiled variable-stratification kernels first. The hydrostatic composite benchmark loses the compiled nonlinear kernel's advantage in tracer derivatives. The output writer is a small cost. A diagnostic substitution of MATLAB `diffX` and `diffY`, with all other compiled operations retained, reduced the profile from **60.815 to 42.607 seconds** and passed complete saved-output comparisons. This is a single exploratory profile pair, not adoption qualification or replacement publication data.

The measured horizontal-derivative difference predicts roughly **26% lower whole-workload time** if the compiled implementation reaches the diagnostic MATLAB throughput. Use about 25% as an experimental hypothesis, not a promised speedup. Prepare bounded batches or parallel plane work using the existing native provider and worker infrastructure; evaluate axis-only transforms because differentiation does not require transforming the other horizontal axis. Preserve the bounded-memory decision in #463. Real vertical calculus is the second target. A broad tracer, adapter, solver or output rewrite is not justified by this audit.
...
- [#463](https://github.com/JeffreyEarly/wave-vortex-model/issues/463), adopted through #465/#466, deliberately replaced full-depth derivative scratch with one plane to bound memory while preserving every resolved frequency. Keep that invariant; do not undo it by blindly restoring full-volume staging. Its original QG 3.385% timing miss remains historical evidence.
...
For implementation, first screen bounded parallel or batched full-frequency horizontal derivatives, then consider axis-only plans if worthwhile. Test both directions, derivative orders 1–4, odd/even grids, Nyquist and frequencies outside the retained coefficient set, real/complex public behavior where supported, aliases/strides, warmed allocation and lifecycle rules, and downstream tracer/control/output agreement. Qualify the reviewed native candidate with frozen builds and three fresh processes per coefficient/composite configuration on an idle host, preserving all samples and investigating changes above 3%. The shared kernel also serves standalone and other stratified families; measure those consumers before claiming their benefit. A real vertical operator can follow as a separate increment.
