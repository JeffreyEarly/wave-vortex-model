...
comparison=repmat(struct("id","","Nxyz",[] ,"isHydrostatic",false,"status","failed","matlabExactRetainedBytes",NaN,"compiledExactRetainedBytes",NaN,"exactRetainedRatio",NaN,"matlabExactRetainedRangeBytes",[] ,"compiledExactRetainedRangeBytes",[] ,"exactRetainedRatioRange",[] ,"matlabOperationPeakIncrementRSSBytes",NaN,"compiledOperationPeakIncrementRSSBytes",NaN,"operationPeakRSSRatio",NaN,"matlabOperationPeakIncrementRSSRangeBytes",[] ,"compiledOperationPeakIncrementRSSRangeBytes",[] ,"operationPeakRSSRatioRange",[] ,"matlabProcessExactRetainedBytes",[] ,"compiledProcessExactRetainedBytes",[] ,"matlabProcessPeakIncrementRSSBytes",[] ,"compiledProcessPeakIncrementRSSBytes",[] ,"libraryIdentityPassed",false,"nativeExecutionPassed",false,"noFallback",false,"planCount",NaN,"persistentFullHermitianBytes",NaN,"referenceCallsDuringSampling",NaN),numel(cases),1);
for iCase=1:numel(cases)
    ...
    exactRatios=compiledExact./matlabExact; rssRatios=compiledRSS./matlabRSS;
    comparison(iCase)=struct("id",string(cases(iCase).id),"Nxyz",cases(iCase).Nxyz,"isHydrostatic",cases(iCase).isHydrostatic,"status",conditional(all(string({selected.status})=="complete"),"complete","failed"),"matlabExactRetainedBytes",median(matlabExact),"compiledExactRetainedBytes",median(compiledExact),"exactRetainedRatio",median(exactRatios),"matlabExactRetainedRangeBytes",[min(matlabExact) max(matlabExact)],"compiledExactRetainedRangeBytes",[min(compiledExact) max(compiledExact)],"exactRetainedRatioRange",[min(exactRatios) max(exactRatios)],"matlabOperationPeakIncrementRSSBytes",median(matlabRSS),"compiledOperationPeakIncrementRSSBytes",median(compiledRSS),"operationPeakRSSRatio",median(rssRatios),"matlabOperationPeakIncrementRSSRangeBytes",[min(matlabRSS) max(matlabRSS)],"compiledOperationPeakIncrementRSSRangeBytes",[min(compiledRSS) max(compiledRSS)],"operationPeakRSSRatioRange",[min(rssRatios) max(rssRatios)],"matlabProcessExactRetainedBytes",matlabExact,"compiledProcessExactRetainedBytes",compiledExact,"matlabProcessPeakIncrementRSSBytes",matlabRSS,"compiledProcessPeakIncrementRSSBytes",compiledRSS,"libraryIdentityPassed",identity,"nativeExecutionPassed",string(metadata.engine)=="fftw","noFallback",~metadata.fallback,"planCount",metrics.planCount,"persistentFullHermitianBytes",metrics.persistentFullHermitianBytes,"referenceCallsDuringSampling",sum([selected.referenceCallCount]));
end
end
...
lines=["# Issue #131 corrected memory reassessment";"";"- Status: `"+results.status+"`";"- Core: `"+fieldOr(results.decision,"coreStatus","pending")+"`";"- Memory: `"+fieldOr(results.decision,"memoryStatus","pending")+"`";"- Integration: `"+fieldOr(results.decision,"integrationStatus","provisional")+"`";"- Frozen speed evidence: artifact `d023626`, JSON SHA-256 `9bf05e1d...`";"";"The prior artifact's timing, correctness, provider, and lifecycle fields remain authoritative. Its memory fields and integration decision are superseded.";"";"## Corrected comparison";"";"Ratios are median paired-process ratios with the observed minimum–maximum range in brackets.";"";"| Case | MATLAB retained (MiB) | C++ retained (MiB) | Retained ratio | MATLAB peak RSS increment (MiB) | C++ peak RSS increment (MiB) | RSS ratio | Frozen speedup |";"|---|---:|---:|---:|---:|---:|---:|---:|"];
if ~isempty(results.comparison)
    for item=results.comparison'
        ...
        exactRange=item.exactRetainedRatioRange; rssRange=item.operationPeakRSSRatioRange;
        lines(end+1)="| "+item.id+" | "+sprintf('%.3f',item.matlabExactRetainedBytes/2^20)+" | "+sprintf('%.3f',item.compiledExactRetainedBytes/2^20)+" | "+sprintf('%.3f [%.3f–%.3f]',item.exactRetainedRatio,exactRange(1),exactRange(2))+" | "+sprintf('%.3f',item.matlabOperationPeakIncrementRSSBytes/2^20)+" | "+sprintf('%.3f',item.compiledOperationPeakIncrementRSSBytes/2^20)+" | "+sprintf('%.3f [%.3f–%.3f]',item.operationPeakRSSRatio,rssRange(1),rssRange(2))+" | "+sprintf('%.3fx',speed)+" |"; %#ok<AGROW>
    end
end
