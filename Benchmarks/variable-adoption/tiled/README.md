...
The independent integration review identified one essential boundary: the fused path must own initial production of the four physical evaluator nodes. Calling it after today's `physicalFields()` would repeat value inverses and lose the screened reuse. The existing `evaluateGroup` can publish all four fields only after successful nonlinear execution. The initial path should be eligible only when all four nodes are empty, the provider/layout supports it, the policy is reuse, projection is requested, and derivative capture/spatial-tendency output is absent. Already/partially ready fields and other consumers retain the established path.

Prepare each target's z spectrum in its final retained flux slot, gather its tile before overwriting that tile's rows, then run unchanged vertical projection and coefficient assembly. This avoids separate full target z storage, but needs an extra retained-spectrum slot for Hydrostatic. Keep the existing physical bundle and kernel workspace until a separate lifetime audit justifies reducing them. The native operation must hold its active-resource guard; evaluator fields and accumulated public flux publish only on success. Preserve actual reconstruction/MM counts and expose separate tiled/reused-column metrics. These are proposed integration constraints, not capabilities already implemented by this benchmark.

Follow-up: [issue #489](https://github.com/JeffreyEarly/wave-vortex-model/issues/489).
