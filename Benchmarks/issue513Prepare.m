function records = issue513Prepare(outputDirectory,options)
% Recreate the two scientifically matched issue 513 benchmark fixtures.
arguments
...
for iWorkload = 1:2
    pathname = fullfile(outputDirectory,"matched-constant-nonhydrostatic-"+workloads(iWorkload)+"-model.nc");
    if isfile(pathname), error("WaveVortexBenchmark:Issue513FixtureExists","Refusing to overwrite fixture: %s",pathname); end
