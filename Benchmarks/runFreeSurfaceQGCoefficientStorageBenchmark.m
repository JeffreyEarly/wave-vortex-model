...
definitions = caseDefinitions(options.gridIds,options.endpointIds,options.smallNxyz,options.representativeNxyz,options.Lxyz);
...
configuration = struct( ...
    ...
    "gramTolerance",options.gramTolerance,"Lxyz",options.Lxyz, ...
...
function definitions = caseDefinitions(gridIds,endpointIds,smallNxyz,representativeNxyz,Lxyz)
grids = [ ...
    struct("id","small","Nxyz",smallNxyz,"Lxyz",Lxyz,"modeCountPolicy","fixed-native-quadrature"); ...
    struct("id","representative","Nxyz",representativeNxyz,"Lxyz",Lxyz,"modeCountPolicy","fixed-native-quadrature")];
endpoints = [ ...
...
definitions = repmat(struct("id","","gridId","","endpointId","","Nxyz",zeros(1,3),"Lxyz",zeros(1,3),"g0",NaN,"gd",NaN,"activeEndpointCount",0,"modeCountPolicy",""),definitionCount,1);
iDefinition = 0;
for gridId = gridIds
    ...
    for endpointId = endpointIds
        ...
        definitions(iDefinition) = struct( ...
            ...
            "modeCountPolicy",grid.modeCountPolicy);
    end
end
end
...
config = struct( ...
    ...
    "gramTolerance",configuration.gramTolerance,"Lxyz",configuration.Lxyz, ...
...
wvt = WVTransformFreeSurfaceQG(definition.Lxyz,definition.Nxyz, ...
    N2Function=@benchmarkStratification,latitude=45,g0=definition.g0,gd=definition.gd, ...
    gramTolerance=configuration.gramTolerance);
...
wvt.Amda = 0.02*randn(size(wvt.Amda))./reshape(1:wvt.mdaModeCount,[],1);
end
...
value = struct( ...
    ...
    "apvModeCount",double(wvt.apvModeCount), ...
    "mdaModeCount",double(wvt.mdaModeCount), ...
    "gramTolerance",wvt.gramTolerance, ...
    "apvGramError",wvt.apvGramError, ...
    "mdaGramError",wvt.mdaGramError, ...
    "quadraticDealiasing",wvt.quadraticDealiasing, ...
    "retainedFraction",wvt.retainedFraction, ...
    "energyFraction",wvt.energyFraction, ...
    "bandwidthFraction",wvt.bandwidthFraction, ...
    "apvLinearCount",wvt.constructionAssessment.apv.linearCount, ...
    "apvFilteringCount",wvt.constructionAssessment.apv.filteringCount);
end
