...
arguments
    ...
    options.horizontalZBatch (1,1) double {mustBeMember(options.horizontalZBatch,[0 16 32 64])} = 0
    options.verticalHalfRowBatch (1,1) double {mustBeMember(options.verticalHalfRowBatch,[0 256 1024 4096])} = 0
end
...
compilerFlags = "CXXFLAGS=$CXXFLAGS -std=c++17 -pthread -O3 -mcpu=native" + ...
    ...
    " -DWV_KERNEL_HORIZONTAL_Z_BATCH="+options.horizontalZBatch + ...
    " -DWV_KERNEL_VERTICAL_HALF_ROW_BATCH="+options.verticalHalfRowBatch;
