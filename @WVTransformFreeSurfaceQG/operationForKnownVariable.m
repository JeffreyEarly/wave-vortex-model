...
% Create one operation sharing reconstruction across requested QG fields.
%
...
% - Parameter variableName: requested QG field names, supplied as separate arguments
...
if isempty(names) || length(unique(names)) ~= length(names) || any(~ismember(names,string(self.namesOfTransformVariables())))
    error('WVTransform:UnknownVariable','Request distinct QG fields listed by namesOfTransformVariables.')
end
